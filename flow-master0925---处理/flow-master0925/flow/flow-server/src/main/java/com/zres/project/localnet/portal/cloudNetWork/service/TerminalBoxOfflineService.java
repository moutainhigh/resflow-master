...
/**
 * @ClassName TerminalBoxOfflineService
 * @Description TODO 2.14	终端盒下线通知接口
 * @Author wang.g2
 ...
 */
@Service
public class TerminalBoxOfflineService implements TerminalBoxOfflineIntf {
    ...
    @Override
    public Map<String, Object> terminalBoxOffline(@RequestParam Map<String, Object> params) {
        Map<String, Object> reqMap = new HashMap<>();
        Map<String, Object> param = new HashMap<>();
        Map<String, Object> retMap = new HashMap<>();
        String srvOrdId = MapUtils.getString(params, "srvOrdId");

        param.put("MDEVICE", MapUtils.getString(params, "MDEVICE")); // 盒子唯一标志MAC地址

        //拼接请求报文
        reqMap= queryCloudNetworkInfo(params);
        reqMap.put("PARAM", param);
        logger.info("云组网终端盒下线通知接口请求报文 ：" + JSONObject.toJSONString(reqMap));

        String url = webServiceDao.queryUrl("terminalBoxOffline");
        Map constructResponse = xmlUtil.sendHttpPostOrderCenter(url, JSONObject.toJSONString(reqMap));
        logger.info("云组网终端盒下线通知接口反馈报文 ：" + JSONObject.toJSONString(constructResponse));
        //插入接口记录
        Map<String, Object> interflog = new HashMap<String, Object>();
        interflog.put("INTERFNAME", "云组网终端盒下线通知接口");
        interflog.put("URL", url);
        interflog.put("CONTENT", JSONObject.toJSONString(reqMap));
        interflog.put("REMARK", "云组网终端盒下线通知接口请求报文");
        ...
        if ("200".equals(code)) {
            ...
            //0：成功；-1：失败；
            String respDesc = MapUtils.getString(respJson, "MESSAGE", "云组网终端盒下线通知接口交互异常");//失败原因 -1时必填
            if ("0".equals(MapUtils.getString(respJson, "CODE"))) {
                ...
                String reason = MapUtils.getString(respJson, "reason", "云组网终端盒下线通知接口成功");//失败原因 -1时必填
                //TODO 反馈前端
