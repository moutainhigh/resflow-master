...
/**
 * Created by tang.huili 省内自建系统 业务报竣接口
 */
@Service
// @Controller
// @RequestMapping("/finishOrder")
public class OssToProvFinishOrderService implements OssToProvFinishOrderServiceIntf {
    private static final Logger logger = Logger.getLogger(OssToProvFinishOrderService.class);
    ...
    public Map<String,Object> ossToProvFinishOrder(Map<String, Object> map) {
       ...
       // String orderId = MapUtils.getString(srvMap, "ORDER_ID");
        ...
        if (BasicCode.DIA_SERVICEID.equals(serviceId) && ("101,102,106".indexOf(activeType)!= -1) && "5".equals(subType)){
            returnMap = this.finishOrder(map);
        }
        return returnMap;
    }
    public Map<String,Object> finishOrder(Map<String, Object> map) {
        ...
        try {
            ...
            // 获取订单中心 -url
            String url = wsd.queryUrl("ossToProvFinishOrder");
            ...
            interflog.put("INTERFNAME", "oss向省内自建系统的业务报竣接口ossToProvFinishOrder");
            ...
            rmap.put("attr_name", "");
            rmap.put("attr_value", "");
            rmap.put("attr_value_name", "业务报竣接口返回结果");
            rmap.put("create_date", df.format(new Date()));
            rmap.put("sourse", "");
            if(!flag){
                rmap.put("attr_action", "ossToProvFinishOrder");
            } else{
                rmap.put("attr_action", "ossToProvFinishOrder_4A");
    ...
    private String createFinishOrderJsonStr(Map<String,Object> map) throws Exception{
        ...
        paraObj.put("PARA_ID", "Test");//      本地测试通知
        paraObj.put("PARA_VALUE", "");
        para.add(paraObj);
