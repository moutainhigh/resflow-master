...
/**
 * @ClassName FullBusinessTestService
 * @Description TODO 2.13	全程业务测试通知接口
 * @Author wang.g2
 ...
 */
@Service
public class FullBusinessTestService implements FullBusinessTestIntf {
    private static Logger logger = LoggerFactory.getLogger(FullBusinessTestService.class);
    ...
    @Override
    public Map<String, Object> fullBusinessTest(@RequestParam Map<String, Object> params) {
        Map<String, Object> reqMap = new HashMap<>();
        Map<String, Object> param = new HashMap<>();
        Map<String, Object> retMap = new HashMap<>();
        String srvOrdId = MapUtils.getString(params, "srvOrdId");

        param.put("TESTCAPACITY", MapUtils.getString(params, "TESTCAPACITY")); //测试能力标识 1371|2544

        //拼接请求报文
        reqMap= queryCloudNetworkInfo(params);
        reqMap.put("PARAM", param);
        logger.info("云组网全程业务测试通知接口请求报文 ：" + JSONObject.toJSONString(reqMap));

        String url = webServiceDao.queryUrl("fullBusinessTest");
        Map constructResponse = xmlUtil.sendHttpPostOrderCenter(url, JSONObject.toJSONString(reqMap));
        logger.info("云组网全程业务测试通知接口反馈报文 ：" + JSONObject.toJSONString(constructResponse));
        //插入接口记录
        Map<String, Object> interflog = new HashMap<String, Object>();
        interflog.put("INTERFNAME", "云组网全程业务测试通知接口");
        interflog.put("URL", url);
        interflog.put("CONTENT", JSONObject.toJSONString(reqMap));
        interflog.put("REMARK", "云组网全程业务测试通知接口请求报文");
        ...
        if ("200".equals(code)) {
            ...
            //0：成功；-1：失败；
            String respDesc = MapUtils.getString(respJson, "MESSAGE", "云组网全程业务测试通知接口交互异常");//失败原因 -1时必填
            if ("0".equals(MapUtils.getString(respJson, "CODE"))) {
                ...
                String reason = MapUtils.getString(respJson, "reason", "云组网全程业务测试通知接口成功");//失败原因 -1时必填
                //TODO 反馈前端
