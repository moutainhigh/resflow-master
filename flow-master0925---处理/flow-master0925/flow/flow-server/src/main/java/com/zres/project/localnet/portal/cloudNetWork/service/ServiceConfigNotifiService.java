...
/**
 * @ClassName ServiceConfigNotifiService
 * @Description TODO
 * @Author wang.g2
 ...
 */
@Service
public class ServiceConfigNotifiService implements ServiceConfigNotifiIntf {

    private static Logger logger = LoggerFactory.getLogger(ServiceConfigNotifiService.class);
    ...
    @Override
    public Map<String, Object> serviceConfigNotifi(@RequestParam Map<String, Object> params) {
        ...
        logger.info("云组网业务配置下发通知接口请求报文 ：" + JSONObject.toJSONString(reqMap));

        String url = webServiceDao.queryUrl("serviceConfigNotifi");
        Map constructResponse = xmlUtil.sendHttpPostOrderCenter(url, JSONObject.toJSONString(reqMap));
        logger.info("云组网业务配置下发通知接口反馈报文 ：" + JSONObject.toJSONString(constructResponse));
        //插入接口记录
        Map<String, Object> interflog = new HashMap<String, Object>();
        interflog.put("INTERFNAME", "云组网业务配置下发通知接口");
        interflog.put("URL", url);
        interflog.put("CONTENT", JSONObject.toJSONString(reqMap));
        interflog.put("REMARK", "云组网业务配置下发通知接口请求报文");
        ...
        if ("200".equals(code)) {
            ...
            //0：成功；-1：失败；
            String respDesc = MapUtils.getString(respJson, "MESSAGE", "云组网本业务配置下发通知接口交互异常");//失败原因 -1时必填
            if ("0".equals(MapUtils.getString(respJson, "CODE"))) {
                ...
                String reason = MapUtils.getString(respJson, "reason", "云组网业务配置下发通知接口成功");//失败原因 -1时必填
                //TODO 反馈前端
