...
/**
 * @ClassName PortSelectionService
 * @Description TODO  下联端口选择接口
 * @Author wang.g2
 ...
 */
@Service
public class PortSelectionService implements PortSelectionIntf {
    private static Logger logger = LoggerFactory.getLogger(PortSelectionService.class);
    ...
    @Override
    public Map<String, Object> portSelection(@RequestParam Map<String, Object> params) {
        ...
        param.put("NETWORK_TYPE", MapUtils.getString(params,"NETWORK_TYPE")); //客户网路类型
        param.put("NETWORK_MODE", MapUtils.getString(params,"NETWORK_MODE")); //组网方式
        param.put("IS_VLAN", MapUtils.getString(params,"IS_VLAN"));//是否划分vlan
        param.put("PORT_TYPE", MapUtils.getString(params,"PORT_TYPE"));//接口类型
        param.put("PORT_NUM", MapUtils.getString(params,"PORT_NUM"));//端口号
        param.put("VLAN_NUM", MapUtils.getString(params,"VLAN_NUM"));//vlan 号

        //拼接请求报文
        reqMap.put("ORDER_INFO", orderInfo);
        reqMap.put("OPERATOR", operator);
        reqMap.put("PARAM", param);
        logger.info("云组网下联端口选择接口请求报文 ：" + JSONObject.toJSONString(reqMap));

        String url = webServiceDao.queryUrl("portSelection");
        ...
        if("200".equals(code)){
            ...
            //0：成功；-1：失败；
            String respDesc = MapUtils.getString(respJson,"MESSAGE","云组网下联端口选择接口交互异常");//失败原因 -1时必填
            if("0".equals(MapUtils.getString(respJson,"CODE"))){
                ...
                String reason = MapUtils.getString(respJson,"reason","云组网下联端口选择接口成功");//失败原因 -1时必填
                //TODO 反馈前端
