...
/**
 * @ClassName PortDataDeletionService
 * @Description TODO  IPRAN 端口数据删除接口
 * @Author wang.g2
 ...
 */
@Service
public class PortDataDeletionService implements PortDataDeletionIntf{

    private static Logger logger = LoggerFactory.getLogger(PortDataDeletionService.class);
    ...
    @Override
    public Map<String,Object>  portDataDeletion(@RequestParam Map<String,Object> params){
        ...
        param.put("city", city);
        param.put("ranLoopNmIp", MapUtils.getString(params,"ranLoopNmIp"));//IPRAN侧管理接口ip
        param.put("force", MapUtils.getString(params,"force"));//yes 标识强制执行

        //拼接请求报文
        reqMap = queryCloudNetworkInfo(params);
        reqMap.put("PARAM", param);
        logger.info("云组网IPRAN 端口数据删除接口请求报文 ：" + JSONObject.toJSONString(reqMap));

        String url = webServiceDao.queryUrl("portDataDeletion");
        Map constructResponse = xmlUtil.sendHttpPostOrderCenter(url, JSONObject.toJSONString(reqMap));
        logger.info("云组网IPRAN 端口数据删除接口请求报文 ：" + JSONObject.toJSONString(constructResponse));
        //插入接口记录
        Map<String,Object> interflog = new HashMap<String, Object>();
        interflog.put("INTERFNAME","云组网IPRAN 端口数据删除接口");
        interflog.put("URL",url);
        interflog.put("CONTENT",JSONObject.toJSONString(reqMap));
        interflog.put("REMARK","云组网IPRAN 端口数据删除接口请求报文");
        ...
        if("200".equals(code)){
            ...
            //0：成功；-1：失败；
            String respDesc = MapUtils.getString(respJson,"MESSAGE","云组网IIPRAN 端口数据删除接口交互异常");//失败原因 -1时必填
            ...
            if("0".equals(MapUtils.getString(respJson,"CODE"))){
                ...
                String reason = MapUtils.getString(respJson,"reason","云组网IIPRAN 端口数据删除接口成功");//失败原因 -1时必填
                //TODO 反馈前端
