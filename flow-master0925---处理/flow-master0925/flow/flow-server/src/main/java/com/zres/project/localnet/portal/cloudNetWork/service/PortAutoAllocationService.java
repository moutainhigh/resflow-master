...
/**
 * @ClassName PortAutoAllocationService
 * @Description TODO 下联端口自动分配接口
 * @Author wang.g2
 ...
 */
@Service
public class PortAutoAllocationService implements PortAutoAllocationIntf{
    private static Logger logger = LoggerFactory.getLogger(PortAutoAllocationService.class);
    @Autowired
    ...
    @Override
    public Map<String,Object> portAutoAllocation(@RequestParam Map<String,Object> params){
        ...
        logger.info("下联端口自动分配接口 ：" + JSONObject.toJSONString(requestMap));
        //IPRAN资源网管验证接口地址
        String url = wsd.queryUrl("portAutoAllocation");
        Map constructResponse = xmlUtil.sendHttpPostOrderCenter(url, JSONObject.toJSONString(requestMap));
        logger.info("下联端口自动分配接口反馈报文 ：" + JSONObject.toJSONString(constructResponse));
        ...
        if("200".equals(code)){
            ...
            //0：成功；-1：失败；
            String respDesc = MapUtils.getString(respJson,"MESSAGE","云组网下联端口自动分配接口交互异常");//失败原因 -1时必填
            if("0".equals(MapUtils.getString(respJson,"CODE"))){
                ...
                String reason = MapUtils.getString(respJson,"reason","云组网下联端口自动分配成功");//失败原因 -1时必填
