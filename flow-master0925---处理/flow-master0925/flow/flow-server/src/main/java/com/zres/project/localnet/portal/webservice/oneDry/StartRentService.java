            ...
            if (qcWoOrderCode != null) { //全程调测通知
                if (SrvOrdList.get(0).get("QCWOORDERCODE") != null && !"".equals(SrvOrdList.get(0).get("QCWOORDERCODE")) && !"null".equals(SrvOrdList.get(0).get("QCWOORDERCODE"))) { //一干回退全程调测
                    ...
                    if (returnList != null && returnList.size() > 0) {
                        for (int td = 0; td < returnList.size(); td++) {
                            ...
                            rentMap.put("srvOrdIdFullCom", returnList.get(td).get("SRV_ORD_ID"));
                            rentMap.put("flag", "一干");
                        ...
                        List<Map<String, Object>> kyqctcWorkId = springJdbcTemplate.queryForList("SELECT W.WO_ID FROM GOM_ORDER O JOIN GOM_BDW_SRV_ORD_INFO G ON G.ORDER_ID = O.ORDER_ID JOIN GOM_WO W ON W.ORDER_ID = O.ORDER_ID JOIN GOM_BDW_SRV_ORD_INFO I ON I.ORDER_ID = O.ORDER_ID JOIN GOM_PS_2_WO_S WS ON WS.ID = W.PS_ID JOIN UOS_TACHE UT ON UT.ID = WS.TACHE_ID WHERE G.TRADE_TYPE_CODE = '" + orderCode + "' AND  G.TRADE_ID = '" + prodInstId + "' AND  W.WO_STATE = '290000111' AND UT.ID = '500001168'");
                        if (kyqctcWorkId != null && kyqctcWorkId.size() > 0) {
                            for (int qc = 0; qc < kyqctcWorkId.size(); qc++) {
                                springJdbcTemplate.update("update gom_wo set wo_state = '290000002'  where wo_id = " + kyqctcWorkId.get(qc).get("WO_ID"));
                            }
                            springJdbcTemplate.update("UPDATE GOM_BDW_SRV_ORD_INFO SET QCWOORDERCODE = '" + qcWoOrderCode + "' WHERE TRADE_TYPE_CODE = '" + orderCode + "' AND TRADE_ID = '" + prodInstId + "'");
                    ...
                    if (kyqctcWorkId != null && kyqctcWorkId.size() > 0) {
                        ...
                        springJdbcTemplate.update("UPDATE GOM_BDW_SRV_ORD_INFO SET QCWOORDERCODE = '" + qcWoOrderCode + "' WHERE TRADE_TYPE_CODE = '" + orderCode + "' AND TRADE_ID = '" + prodInstId + "'");
                        returnValue = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><root><resultFlag>0</resultFlag><resultContent>全程调测通知成功</resultContent></root>";interflog.put("RETURNCONTENT", returnValue);
                        wsd.updateInterfLog(interflog); //修改日志表返回结果
                        return returnValue;
                    }
                }
                ...
                if (rentList != null && rentList.size() > 0) {
                    ...
                    for (int qz = 0; qz < rentList.size(); qz++) {
                        ...
                        //rmap.put("attr_value", df.format(startTime));
