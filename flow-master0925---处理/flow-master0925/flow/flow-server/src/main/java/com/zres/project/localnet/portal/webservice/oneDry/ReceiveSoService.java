...
public class ReceiveSoService implements ReceiveSoServiceIntf {
    ...
    public String receiveSo(@RequestBody String xml) {
        ...
        String tradeTypeCode = MapUtils.getString(serviceOrderInfo, "groupOrderCode");//获取一干关联原单的标识
        ...
        wsd.insertInterfLogS(interflog);
        if ("114".equals(serviceOrderInfo.get("orderType"))) { //全程调测退单
            for (int tddl = 0; tddl < prodInfo.size(); tddl++) {
                List<Map<String, Object>> returnList = springJdbcTemplate.queryForList("SELECT * FROM (SELECT W.WO_ID,O.ORDER_ID,WS.ORD_PS_ID,UT.ID,UT.TACHE_NAME,G.MAINORG,G.SRV_ORD_ID,ROWNUM rn FROM GOM_ORDER O JOIN GOM_BDW_SRV_ORD_INFO G ON G.ORDER_ID = O.ORDER_ID JOIN GOM_WO W ON W.ORDER_ID = O.ORDER_ID JOIN GOM_BDW_SRV_ORD_INFO I ON I.ORDER_ID = O.ORDER_ID JOIN GOM_PS_2_WO_S WS ON WS.ID = W.PS_ID JOIN UOS_TACHE UT ON UT.ID = WS.TACHE_ID WHERE G.TRADE_ID = '" + prodInfo.get(tddl).get("prodInstId") + "' AND  G.TRADE_TYPE_CODE = '" + serviceOrderInfo.get("orderCode") + "' AND  ((W.WO_STATE = '290000006' AND UT.ID = 500001168) OR (W.WO_STATE = '290000002' AND UT.ID = 500001222)) ORDER BY G.CREATE_DATE,W.CREATE_DATE DESC) WHERE RN = 1");
                List<Map<String, Object>> mainOrg = springJdbcTemplate.queryForList("SELECT SORT_NO AS AREA_ID FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='PROVINCE' AND CODE_TYPE_NAME='" + header.get("province") + "' AND REMARK = '" + prodInfo.get(tddl).get("mainOrg") + "'"); //查询主调局编码
                if (judgeNULL(returnList)) {
                    if (judgeNULL(mainOrg) && returnList.get(0).get("MAINORG").equals(mainOrg.get(0).get("AREA_ID"))) { //如果此单主调局是本省只改电路表的FLOW_ID（给一干回单的工单ID）
                        ...
                        rentMap.put("srvOrdIdFullCom", returnList.get(0).get("SRV_ORD_ID"));
                        rentMap.put("flag", "一干");
            ...
            String province = MapUtils.getString(header, "province");
            for(Map<String, Object> prodInfoMap : prodInfo){
                ...
                for (int ycxx = 0; ycxx < attr.size(); ycxx++) {
                    ...
                    if ("crm_prod_inst_id".equals(prods.get("code"))) {
                        if(yc==0){
                            crm_prod_inst_id.append("'").append(prods.get("value")).append("'");
                        }else {
                            crm_prod_inst_id.append(",'").append(prods.get("value")).append("'");
                        }
                        ...
                        if(judgeNULL(originalList) && judgeNULL(originalList.get(0))) {
                            ...
                            if ("108".equals("" + serviceOrderInfo.get("orderType")) || "109".equals("" + serviceOrderInfo.get("orderType"))) { //加急或延期
                                List<Map<String, Object>> woList = springJdbcTemplate.queryForList("SELECT WO.WO_ID FROM GOM_WO WO LEFT JOIN GOM_PS_2_WO_S WS  ON WS.ID = WO.PS_ID LEFT JOIN UOS_TACHE UT ON UT.ID = WS.TACHE_ID WHERE WS.TACHE_ID = '500001160' AND WO.WO_STATE = '290000002'  AND ORDER_ID =" + orderId);
                                if (!judgeNULL(woList)) {
                                    ...
                                    rentMap.put("remark", "一干下发本地加急延期需要回退到本地测试环节进行二次报竣");
...
//                              srvOrdStat="10X";
                            ...
                            else if ("104".equals("" + serviceOrderInfo.get("orderType"))) { //追单
                                ...
                                List<Map<String, Object>> countWoList = springJdbcTemplate.queryForList("SELECT WO.WO_ID FROM GOM_WO WO LEFT JOIN GOM_PS_2_WO_S WS ON WS.ID = WO.PS_ID LEFT JOIN UOS_TACHE UT ON UT.ID = WS.TACHE_ID WHERE WS.TACHE_ID = '500001160' AND ORDER_ID =" + orderId);
                                if (judgeNULL(countWoList)) {
                                    ...
                                    if (!judgeNULL(woList)) {
                                        ...
                                        rentMap.put("orderId", orderId);
                                        rentMap.put("srvOrdId", MapUtils.getString(originalList.get(0), "SRV_ORD_ID"));
                                        rentMap.put("remark", "一干下发本地追单需要回退到本地测试环节进行二次报竣");
                ...
                prodInfoDTO.setOnedryAreaCode(province);
            ...
            for (int dl = 0; dl < prodInfo.size(); dl++) {
                ...
                List<Map<String, Object>> services = springJdbcTemplate.queryForList("SELECT SORT_NO FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='flow_cross' AND CODE_TYPE_NAME='" + prodType + "' AND CODE_VALUE='" + prodInfo.get(dl).get("actType") + "' AND CODE_CONTENT='" + serviceOrderInfo.get("orderType") + "'");
                ...
                if (!judgeNULL(serviceOrderInfo.get("orderTitle"))) {
                    ...
                    Map<String, Object> province = springJdbcTemplate.queryForMap("SELECT CODE_VALUE AS REGION_ID FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='PROVINCE' AND CODE_TYPE_NAME='" + header.get("province") + "' AND NVL(CODE_VALUE,'-1')!='-1'");
                ...
                //header.get("province") 为50668香港是查询GOM_BDW_CODE_INFO表中的ORDER_NO字段其他查询SORT_NO字段为主调局
                List<Map<String, Object>> zdj = null;
                if("50668".equals(MapUtils.getString(header,"province"))){
                    zdj = springJdbcTemplate.queryForList("SELECT ORDER_NO AS AREA_ID FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='PROVINCE' AND REMARK = '" + prodInfo.get(dl).get("mainOrg") + "'");
                } else {
                    zdj = springJdbcTemplate.queryForList("SELECT SORT_NO AS AREA_ID FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='PROVINCE' AND CODE_TYPE_NAME='" + header.get("province") + "' AND REMARK = '" + prodInfo.get(dl).get("mainOrg") + "'");
                }
                ...
                addgbsoiSql.append("'" + serviceOrderInfo.get("orderType") + "','10N','onedry','" + prodInfo.get(dl).get("omOrderCode") + "','" + crmProdInstId + "','" + crmProdInstId + "',sysdate,'"+requFineTime+"')");
                ...
                Map<String, Object> slAREA = springJdbcTemplate.queryForMap("SELECT SORT_NO AS SLAREA FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='PROVINCE' AND CODE_TYPE_NAME='" + header.get("province") + "' AND NVL(CODE_VALUE,'-1')!='-1'"); //查询受理区域
                List<Map<String, Object>> aAREA = springJdbcTemplate.queryForList("SELECT CODE_VALUE AS AAREA FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='CITY' AND CODE_TYPE_NAME='" + citya + "' AND SORT_NO='" + slAREA.get("SLAREA") + "'"); //查询A端所属区域
                List<Map<String, Object>> zAREA = springJdbcTemplate.queryForList("SELECT CODE_VALUE AS ZAREA FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='CITY' AND CODE_TYPE_NAME='" + cityz + "' AND SORT_NO='" + slAREA.get("SLAREA") + "'"); //查询Z端所属区域
                if (judgeNULL(aAREA)) {
        ...
        if (judgeNULL(repeatList)) {
            ...
            Map<String, Object> receptionArea = springJdbcTemplate.queryForMap("SELECT CODE_VALUE,CODE_CONTENT FROM GOM_BDW_CODE_INFO WHERE CODE_TYPE='PROVINCE' AND CODE_TYPE_NAME='" + header.get("province") + "' AND NVL(CODE_VALUE,'-1')!='-1'");
            ...
            wsd.updateInterfLog(interflog);
            if(judgeNULL(attachment)) {
                HandleAttachment ha = new HandleAttachment(attachment, "JT_FTP_INFO", "FTP_INFO", "ONEDRY"); //创建异步处理附件
                ha.start(); //启动异步处理附件
            }
            if (!ListUtil.isEmpty(orderIdList)){
                logger.info("开始准备发送短信。。。。。。。。。。。。。。。。。。。。");
