define([
    'text!module/UnicomLocalNet/resmaster/portal/cloudNetworkFlow/templates/fiberCheckChildFlowView.html',
...
], function (fiberCheckChildFlowView, cloudNetWorkResCheckFlowAction, operOrderAction, css) {
    return fish.View.extend({
        template: fish.compile(fiberCheckChildFlowView),
        events: {
        ...
        initCircuitInfo: function () {
            ...
            $("#fiberChildFlowGrid").grid("setGridHeight", 150);
        ...
        initCheckInfoValue:function(){
            var data = $("#fiberChildFlowGrid").grid("getSelection");
        ...
        qrycircuitInfo: function () {
            ...
            cloudNetWorkResCheckFlowAction.qrycircuitInfo(param, function(res){
                if (res.success){
                    $("#fiberChildFlowGrid").grid("reloadData", res.data);
                }else {
        ...
        submit: function () {
            var me = this;
            var circuitData = $("#fiberChildFlowGrid").grid("getCheckRows");//订单信息
            if (circuitData != null && circuitData.length < 1){
                fish.info("请先选择要提交的电路！");
                return;
            }
            //校验勾选的电路是否有未进行核查信息保存的
        ...
        saveCheckInfo:function(){
            ...
            var resAccessA = $("#A_RES_SATISFY").val();
            if (resAccessA == null || resAccessA == ''){
                fish.info("A端光纤资源是否满足不能为空！");
                return;
            }
            var resAccessZ = $("#Z_RES_SATISFY").val();
            if (resAccessZ == null && resAccessZ == ''){
                fish.info("Z端光纤资源是否满足不能为空！");
                return;
            }
            if (resAccessA == '0' || resAccessZ == '0'){
