define(['text!module/UnicomLocalNet/resmaster/portal/checkFlow/templates/checkDispatchViewDouble.html',
    ...
    return fish.View.extend({
       ...
       //     this.initValidate();
        ...
        initCheckInfo : function (param) {
            ...
            let tacheId = meOpera.options.tacheId;
            if(checkFlag) {
                let feedBackSign = param.FEEDBACKSIGN;
                // 核查汇总、投资估算、核查调度
                switch (tacheId) {
                    case "500001150":
                    case "500001151":
                    case "500001144":
                        ...
                        break;
                    case "500001144":
                        let ifTemp1 = sysResource.SYSTEM_RESOURCE=="second-schedule-lt" && $('#orderOper-form').form('value').resRadio == 1;
                        if (ifTemp1) {
                            feedBackSign = "Z";
                        }
                        break;
                    case "500001150":
                    case "500001151":
                        let ifTemp2 = sysResource.SYSTEM_RESOURCE=="second-schedule-lt";
                        if (ifTemp2) {
                            feedBackSign = "Z";
                        }
                }
                ...
                // // 如果是互联网专线产品，只展示一端
                // let serviceId = param.SERVICE_ID;
                // feedBackSign = serviceId=="10000011"?"Z":feedBackSign;
                // switch (feedBackSign) {
                //
                // }
            }
        ...
        // A端口 Z端口显示
        // A端口 Z端口显示
        initTerminateNameShow:function() {
            switch (this.feedbackSign) {
                case "A":
                    if (meOpera.options.btnFlag != "resConfig" && meOpera.options.btnFlag != "trans") {
                        $("#feedBackADiv").show();
                    }
                    $("#feedBackZDiv").hide();
                    $(".A_TERMINATE").text("");
      ...
      //      meOpera.$orderOperaFrom.validator("setField", rules);
        ...
        //     if((feedBackSign=="A" || feedBackSign=="ALL") && '10000011'!=meOpera.options.serviceId){
        ...
        //         let A_RES_PROVIDE = $('#A_RES_PROVIDE').val();
        //         let Z_RES_PROVIDE = $('#Z_RES_PROVIDE').val();
        //         let A_RES_ACCESS = $('#A_RES_ACCESS').val();
        //         let Z_RES_ACCESS = $('#Z_RES_ACCESS').val();
        //         let A_EQUIP_READY = $('#A_EQUIP_READY').val();
        //         let Z_EQUIP_READY = $('#Z_EQUIP_READY').val();
        //         let ResSatisfyZ = $('#Z_RES_SATISFY').val();//满足-不满足
        //         if(isValidSign && Z_CONSTRUCT_SCHEME==null && ResSatisfyZ == 1){
        //             if('10000011' == meOpera.options.serviceId){
        ...
        //         }else{
        //             //添加的资源与备货情况校验数据不能为空
        //             if(tacheId == "500001150"&&(A_RES_PROVIDE==""||Z_RES_PROVIDE=="")&& '10000011' != meOpera.options.serviceId){
        //                 fish.warn('资源提供方式不能为空！');
        //                 return false;
        //             }else if(tacheId == "500001150"&& Z_RES_PROVIDE=="" && '10000011' == meOpera.options.serviceId){
        //                 fish.warn('资源提供方式不能为空！');
        //                 return false;
        //             }
        //             if(tacheId == "500001150"&&(A_RES_ACCESS==""||Z_RES_ACCESS=="")&& '10000011' != meOpera.options.serviceId){
        //                 fish.warn('资源接入方式不能为空！');
        //                 return false;
        //             }else if(tacheId == "500001150"&& Z_RES_ACCESS=="" && '10000011' == meOpera.options.serviceId){
        //                 fish.warn('资源接入方式不能为空！');
        //                 return false;
        //             }
        //             if(tacheId == "500001150"&&(A_EQUIP_READY==""||Z_EQUIP_READY=="")&& '10000011' != meOpera.options.serviceId){
        //                 fish.warn('备货情况不能为空！');
        //                 return false;
        //             }else if(tacheId == "500001150"&& Z_EQUIP_READY=="" && '10000011' == meOpera.options.serviceId){
        //                 fish.warn('备货情况不能为空！');
        //                 return false;
        //             }
        //         }
