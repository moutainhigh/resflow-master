define(['text!module/UnicomLocalNet/resmaster/portal/checkFlow/templates/pop/checkInfoFeedbackDirect.html',
    ...
    return fish.View.extend({
        ...
        queryCheckInfo : function (param) {
            ...
            orderDetailsAction.queryCheckInfoBack(queryParam,function (res) {
                ...
                for(let key in dataArray){
                    let value = dataArray[key];
                    if (key == "ACCESS_CIR_TYPE"){
                        $("#Z_ACCESS_CIR_TYPE").combobox('value',value);
                    }
                }
                for(let key in dataArray){
                    let value = dataArray[key];
                    if (key.indexOf("_RES_PROVIDE_STAND")  != -1
                        ...
                        || key.indexOf("_TRANS_TYPE")  != -1 || key.indexOf("_OPTICAL_READY")  != -1
                        || key.indexOf("_MUNICIPAL_APPROVAL")  != -1  || key.indexOf("_PROPERTY_REDLINE")  != -1
                        || key.indexOf("_RES_HAVE")  != -1 || key == "Z_ACCESS_CIR_TYPE"
                    ) {
                        // 选择框已赋值
                    } else if (key=="OTHER_ACE_CIR_TYPE" || key == "UPLINK_NODE_PORT"){
                        //电路比较特殊
                        $("#Z_" + key).val(value);
                    } else {
                        $("#"+key).val(value);
        ...
        feedBack:function () {
            ...
            var fileData = $("#Z_fileGrid").grid("getRowData");
            var delFilesStr = "";
        ...
        //        }
                ...
                params.formValue = meOpera.$orderOperaFrom.form("value");
                params.formValue.OTHER_ACE_CIR_TYPE = params.formValue.Z_OTHER_ACE_CIR_TYPE;
                params.formValue.UPLINK_NODE_PORT = params.formValue.Z_UPLINK_NODE_PORT;
                params.formValue.ACCESS_CIR_TYPE = params.formValue.Z_ACCESS_CIR_TYPE;

                params.formValue.Z_RES_HAVE = $("#Z_RES_HAVE").combobox("value");
                params.formValue.Z_TOTAL_AMOUNT = $("#Z_TOTAL_AMOUNT").val();
                params.formValue.Z_LONGEST_PERIOD = $("#Z_LONGEST_PERIOD").val();
                console.error("提交的信息");
                console.error(params);
                $.blockUI({message: '信息反馈中...'});
                if (FILES != null) {
                    me.fileUpdate(params); //上传附件
                }
                me.feedBackDirect(params);
            }
        },
        feedBackDirect : function(params){
            let me = this;
            if (!me.isSubmit) {
                me.isSubmit = true;
                params.newCreateResource = "1";
                orderDetailsAction.feedBackDirect(params, function (res) {
                    me.isSubmit = false;
                    if (res.success) {
                        $.unblockUI();
        ...
        fileUpdate :function (params){
            FILES.url = URL+"/localScheduleLT/FlieUpdateController/uploadFilesNoSubmit.spr";
            FILES.formData = {
                    ...
                    operOrderAction.getAnnex(param,function (data) {
                        ...
                        if (data.length != 0) {
                            for (i = 0; i < data.length; i++) {
                                ...
                                $("#Z_fileGrid").grid("addRowData", fileObj);
                            }
                        }
                    });
                }
            });
            me.$('#Z_attach').fileupload({
                dataType: 'json',
                autoUpload: false,
                add: function(e, data) {
