define(["text!module/UnicomLocalNet/resmaster/portal/orderLocalStandby/templates/LocalStandby.html",
    ...
    return ngc.View.extend({
            ...
            events: {
                ...
                'click #tabs-a-a-link': 'aaClick',      //部门待签收
                'click #tabs-a-b-link': 'abClick',      //岗位待签收
                'click #tabs-a-c-link': 'acClick',      //个人待签收
                'click #tabs-a-d-link': 'adClick',      //处理中
                'click #tabs-a-e-link': 'aeClick',      //已完成
                'click #tabs-a-f1-link': 'af1Click',   //异常单
                'click #tabs-a-g-link': 'agClick',   // 抄送单
            ...
            /**
             *  消息列表跳转页面URL参数获取
             *  @author wangsen 2020年10月8日 17:25:00
             */
            ...
            afterRender: function () {
                ...
                if (srvOrdId && srvOrdId != "") {
                    ...
                    this.queryObj.srvOrdId = $("#srvOrdId").val();
                    this.initorderDealGridGroup();
                    this.queryWorkOrdersGroup();
                }
            },
               ...
               // this.exceptionOrder = true;
                ...
                if (this.queryObj.queryTypeLocal == 'abnormalOrder') { //异常单
                    this.girdMdel = [
                        ...
                        {name: 'APPLY_ORD_ID', label: '申请单编码', width: 110},
                        {name: 'APPLY_ORD_NAME', label: '申请单标题', width: 250},
                        {name: 'CUST_NAME_CHINESE', label: '客户名称', width: 200},
                        {name: 'SUBSCRIBE_ID', label: '客户订单号', width: 100, align: 'center'},
                        {name: 'SPC_TAC_NAME', label: '环节名称', width: 90, align: 'left'},
                        ...
                        {name: 'DISPATCH_ORDER_NO', label: '调度单编号', width: 200, align: 'center'},
                        {name: 'DISPATCH_TITLE', label: '调度单标题', width: 250},
                    ...
                    this.girdMdel = [
                        ...
                        {name: 'SPC_TAC_NAME', label: '环节名称', width: 90, align: 'left'},
                        {name: 'COUNTS', label: '电路数量', width: 80, align: 'center'},
                        {name: 'SERVICE_ID', label: '产品类型', width: 80, hidden:true},
                        {name: 'SERVICETYPE', label: '产品类型', width: 110, formatter: me.formatServiceId},
...
/*
            ...
            formatServiceId: function (value) {
                var serviceIdMap = {
                    ...
                    '80000466' : '政企精品网业务',
                    '10003406' : 'SD-WAN智选专线',
                    '10000011-0' : '互联网专线(DIA)-普通互联网专线',
                    '10000011-1' : '互联网专线(DIA)-国际精品网',
                    '10000011-2' : '互联网专线(DIA)-智慧沃企（增强版）',
                    '10000011-3' : '互联网专线(DIA)-高清互动电视',
                    '10000011-5' : '互联网专线(DIA)-商务专线',
                    '10000011-6' : '互联网专线(DIA)-智享专线',
                    '10000011-7' : '互联网专线(DIA)-尊享专线',
                    '80000465' : '云组网'
                };
            ...
            initorderDealSubGridGroup: function (subGridId, parentRowId, meTemp) {
                ...
                if (meTemp.queryObj.queryTypeLocal != 'dispConfirm') {
                    ...
                    subModel = [
                        ...
                        {name: 'DISPATCH_TITLE', label: '调度单标题', width: 200},
                        {name: 'APPLY_ORD_NAME', label: '申请单标题', width: 200},
                        {name: 'ALARM_DATA', label: '环节预警时间', width: 110, align: 'center'},
                        {name: 'REQ_FIN_DATE', label: '环节要求完成时间', width: 160, align: 'center'},
                        {name: 'ONE_REQ_FIN_DATE', label: '本省端要求完成时间', width: 160, align: 'center'},
                        {name: 'REQ_FIN_DATE_ORDER', label: '全程要求完成时间', width: 160, align: 'center'},
                    ]
...
*/
                ...
                collQueryObj.srvOrdIds=srvOrdIds;
            ...
            queryWorkOrdersGroup: function (page, rowNum, sortname, sortorder) {
                ...
                var standbyType = this.queryObj.standbyType;
            ...
            applyCheck: function(){
                ...
                var popFile = fish.popupView({
                    url: 'module/UnicomLocalNet/resmaster/portal/orderLocalStandby/views/applyCheckView',
                    width: "65%",
                    height: "90%",
                    title: "审核意见",
                    viewOption: {
            ...
            standbyOrderInfoGroup: function (meInfo) {
                ...
                orderStandbyAction.qryCstOrdList(meInfo.queryObj, function (data) {
                    ...
                    if (data.messages = "success") {
                        switch (meInfo.queryObj.queryTypeLocal) {
                            case 'deptStandny':
                                $('#deptStandny').html(data.dataLength);
                                break;
                            case 'jobStandby':
                                $('#jobStandby').html(data.dataLength);
                                break;
                            case 'staffStandby':
                                $('#staffStandby').html(data.dataLength);
                                break;
                            case 'dealOrder':
                                $('#dealOrder').html(data.dataLength);
                                break;
                            case 'dispConfirm': //完成确认单
                                $('#dispConfirm').html(data.dataLength);
                                break;
                            case 'ccOrder':
                                $('#ccOrder').html(data.dataLength);
                                break;
                            case 'abnormalOrder':
                                $('#abnormalOrder').html(data.dataLength);
                                break;
                            case 'applyOrder':
        ...
        cc_confirm:function () {
            ...
            // 优化待办查询 modify by wang.gang2  ①单击 调用查询数据方法 ②签收 查询所在待办以及处理中个数 ③释放签收   查询处理中以及所有待办
