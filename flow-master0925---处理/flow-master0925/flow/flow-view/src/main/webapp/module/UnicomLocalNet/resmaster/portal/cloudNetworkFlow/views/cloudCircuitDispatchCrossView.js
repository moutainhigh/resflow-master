define([
    'text!module/UnicomLocalNet/resmaster/portal/cloudNetworkFlow/templates/cloudCircuitDispatchCrossView.html',
    ...
    return fish.View.extend({
        ...
        savePropertyConfig : function(){
            ...
            var newResourceFlag = $("input[name='newResRadio']:checked").val();
            var specialtyConfigAll = {};
            var specialtyConfigNameAll = {};
            if (newResourceFlag == '0') {
                ...
                if (!specialtyArr[0] && !specialtyArr[1] && !specialtyArr[2]) {
                    ...
                    $.each(specialtyArr,function(i,val){
                        if (val){
                            switch(val) {
                                case 'YZW_OPTICAL':
                                    var opticalCfg = $('#optical_CfgPopedit').popedit('getValue');
                                    if(opticalCfg.value != "" && opticalCfg != undefined){
                                        specialtyConfig.YZW_OPTICAL = opticalCfg.value;
                                        specialtyConfigName.YZW_OPTICAL = opticalCfg.name;
                                    }else {
                                        flag = true;
                                        fish.toast('warn', "光纤专业至少选择一个派发区域！");
                                        return false;
                                    }
                                    break;
                                case 'YZW_IPRAN':
                                    var ipranCfg = $('#ipran_CfgPopedit').popedit('getValue');
                                    if(ipranCfg.value != "" && ipranCfg != undefined){
                                        specialtyConfig.YZW_IPRAN = ipranCfg.value;
                                        specialtyConfigName.YZW_IPRAN = ipranCfg.name;
                                    }else {
                                        flag = true;
                                        fish.toast('warn', "传输ipran专业至少选择一个派发区域！");
                                        return false;
                                    }
                                    break;
                                case 'YZW_MCPE':
                                    var mcpeCfg = $('#mcpe_CfgPopedit').popedit('getValue');
                                    if(mcpeCfg.value != "" && mcpeCfg != undefined){
                                        specialtyConfig.YZW_MCPE = mcpeCfg.value;
                                        specialtyConfigName.YZW_MCPE = mcpeCfg.name;
                                    }else {
                                        flag = true;
                                        fish.toast('warn', "终端盒至少选择一个派发区域！");
                                        return false;
                                    }
                                    break;
                            }
                ...
                specialtyConfigAll.specialtyConfig = specialtyConfig;
                specialtyConfigNameAll.specialtyConfigName = specialtyConfigName;
                param.constructFlag = constructFlag;
            }
            document.getElementById("saveBtn").setAttribute("disabled", true);
            param.specialtyConfig = specialtyConfigAll;
            param.specialtyConfigName = specialtyConfigNameAll;
            ...
            operOrderAction.queryPropertyConfig(param,function(res){
                if (res.success){
                    if (res.configInfo != null){
                        ...
                        //回显派发区域
                        if (res.configInfo.specialtyConfig != null){
                            //回显光纤,传输ipran,终端盒的派发区域
                            $.each(res.configInfo.specialtyConfig,function(i,val){
                                switch(i) {
                                    case 'YZW_IPRAN':
                                        $("#YZW_IPRAN_Checkbox").prop("checked", true);
                                        $("#ipran_CfgPopedit").popedit('setValue', {name:res.configInfoName.specialtyConfigName.YZW_IPRAN, value:val});
                                        break;
                                    case 'YZW_OPTICAL':
                                        $("#YZW_OPTICAL_Checkbox").prop("checked", true);
                                        $("#optical_CfgPopedit").popedit('setValue', {name:res.configInfoName.specialtyConfigName.YZW_OPTICAL, value:val});
                                        break;
                                    case 'YZW_MCPE':
                                        $("#YZW_MCPE_Checkbox").prop("checked", true);
                                        $("#mcpe_CfgPopedit").popedit('setValue', {name:res.configInfoName.specialtyConfigName.YZW_MCPE, value:val});
                                        break;
                                }
                            });
                        }else {
