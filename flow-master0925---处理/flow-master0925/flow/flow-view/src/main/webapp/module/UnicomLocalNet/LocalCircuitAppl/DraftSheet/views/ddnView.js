define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    ...
    return fish.View.extend({
        ...
        initGred:function (mydata) {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    { name: 'acceptArea',label: '受理区域',width: 100,sorttype: "string",align:"center",},
                    { name: 'acceptAreaID',label: '受理区域ID',width: 100,sorttype: "string",align:"center",hidden:true},
            ...
            //添加电路按钮
             $("#addAndSelectRow").off('click').on('click',function(e) {
                ...
                if (actType!="101" && actType!="107"){
                    $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', true);
                }else{
                    $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', true);
                }
                var checkNull = $("#ddn-Form").isValid();
                if (actType!="101" && actType!="107"){
                    $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', false);
                }else{
                    $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', false);
                }
                if(!checkNull){
                ...
                digitalCircutAction.checkTradeId({tradeId:$("#tradeId").val(),serialNumber:$("#serialNumber").val(),actType:actType,orderType:orderType.value},function (res) {
                    ...
                    digitalCircutAction.checkCircuitCode(param, function(res){
                        if(res.success){
                            ...
                            var circuitInfo = {};
                            var circuitCodeArea =['tradeId','serialNumber','serviceId','circuitReq','acceptArea','acceptAreaID','circuitCode','oldCircuitCode','speed','custManager','custManaPhone','requFineTime','accepTime','createTime','slaFlag', 'slaServOpen', 'slaNetQuAss', 'slaSaleServ', 'proPriDeg', 'cirLeaseRange', 'cirUse', "serviceName","circuitReqName","speedName", 'cir_remark', 'ddn_bussiness_type', 'SingleAndDoubleToPVC', 'VPorVC', 'AZRateSCR', 'ZARateSCR', 'AZRatePCR', 'ZARatePCR', 'A_belong_province', 'A_belong_city', 'A_belong_county', 'A_belong_region', 'A_customer_name', 'A_installed_add', 'A_contact_man', 'A_contact_tel', 'A_customer_manager', 'A_customer_manager_tel', 'A_interface_type', 'A_old_belong_region', 'A_old_belong_county', 'A_old_installed_add', 'Z_belong_province', 'Z_belong_city', 'Z_belong_county', 'Z_belong_region', 'Z_customer_name', 'Z_installed_add', 'Z_contact_man', 'Z_contact_tel', 'Z_customer_manager', 'Z_customer_manager_tel', 'Z_interface_type', 'Z_old_belong_region', 'Z_old_belong_county', 'Z_old_installed_add','A_belong_region_id','Z_belong_region_id','A_old_belong_region_id','Z_old_belong_region_id','INSTANCE_ID'];
        ...
        submit:function () {
            ...
            $('#ddn-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,A_belong_region,Z_belong_region' +
