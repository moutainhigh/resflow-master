define([
    'text!module/UnicomLocalNet/resmaster/portal/cloudNetworkFlow/templates/terminalBoxCheckChildFlowView.html',
...
], function (terminalBoxCheckChildFlowView, cloudNetWorkResCheckFlowAction, operOrderAction, css) {
    return fish.View.extend({
        template: fish.compile(terminalBoxCheckChildFlowView),
        events: {
        ...
        afterRender: function () {
            ...
            //是否有MCPE终端盒 选择事件
            $("#mcpeTerminal").on('combobox:change', function () {
                var mcpeTerminal = $("#mcpeTerminal").val();
                if (mcpeTerminal == '1'){
                    $(".terminal").show();
                }else{
                    $(".terminal").hide();
        ...
        initCircuitInfo: function () {
            ...
            $("#terminalChildFlowGrid").grid("setGridHeight", 150);

        },
        initCheckInfoValue:function(){
            var data = $("#terminalChildFlowGrid").grid("getSelection");
            if (data != null || data != undefined || data != {}){
                ...
                cloudNetWorkResCheckFlowAction.queryCheckInfo(param, function(res){
                    if (res.success){
                        // $("#A_RES_SATISFY").val(res.data.A_RES_SATISFY);
                        // $("#Z_RES_SATISFY").val(res.data.Z_RES_SATISFY);
                        // $("#upDeviceTypeA").val(res.data.A_OTHER_TYPE);
                        // $("#upDeviceTypeZ").val(res.data.Z_OTHER_TYPE);
        ...
        qrycircuitInfo: function () {
            ...
            cloudNetWorkResCheckFlowAction.qrycircuitInfo(param, function(res){
                if (res.success){
                    $("#terminalChildFlowGrid").grid("reloadData", res.data);
                }else {
        ...
        submit: function () {
            var me = this;
            var circuitData = $("#terminalChildFlowGrid").grid("getCheckRows");//订单信息
            ...
            if (circuitData != null && circuitData.length > 0){
                for (var i = 0; i < circuitData.length; i++){
                    if (circuitData[i].STATE == '未保存'){
                        fish.info("选择电路中，有未进行核查信息保存的，请重新选择！");
                        return;
                    }
                }
            }
        ...
        saveCheckInfo: function(){
            ...
            var circuitData = $("#terminalChildFlowGrid").grid("getCheckRows");//订单信息
            if (circuitData != null && circuitData.length < 1){
                fish.info("请先选择要保存核查信息的电路！");
                return;
            }
            var mcpeTerminal = $("#mcpeTerminal").val();
            if (mcpeTerminal == null || mcpeTerminal == ''){
                fish.info("是否有MCPE终端盒不能为空！");
                return;
            }
            var terminalSituation = $("#terminalSituation").val();
            if (terminalSituation == null || terminalSituation == ''){
                fish.info("终端情况不能为空！");
                return;
            }
            if (mcpeTerminal == '1'){
                //校验终端盒序列号
                var terminalSeqNum = $("#terminalSeqNum").val();
                if (terminalSeqNum == null || terminalSeqNum == ''){
                    fish.info("终端盒序列号不能为空！");
                    return;
                }
            }
            if (mcpeTerminal == '0'){
                ...
                if (distance == null || distance == ''){
                    fish.info("MCPE地址到上联设备接入机房之间距离不能为空！");
                    return;
                }
            }
            //校验资源情况描述
            if (terminalSituation == '不具备'){
            ...
            if ($("#mcpeTerminal").val() == '1'){
                checkInfo.isOnline = $("#isOnline").val();
                checkInfo.downPort = $("#downPort").val();
                checkInfo.portState = $("#portState").val();
                checkInfo.portRate = $("#portRate").val();
            }
