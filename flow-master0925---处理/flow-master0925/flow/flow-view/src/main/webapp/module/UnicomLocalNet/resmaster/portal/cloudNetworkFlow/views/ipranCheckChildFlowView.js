define([
    'text!module/UnicomLocalNet/resmaster/portal/cloudNetworkFlow/templates/ipranCheckChildFlowView.html',
...
], function (ipranCheckChildFlowView, cloudNetWorkResCheckFlowAction, operOrderAction, css) {
    return fish.View.extend({
        template: fish.compile(ipranCheckChildFlowView),
        events: {
        ...
        initCircuitInfo: function () {
            ...
            $("#ipranChildFlowGrid").grid("setGridHeight", 150);
        ...
        initColModel: function(){
            return [
                ...
                {name: 'TACHE_ID', label: '环节ID', width: 100 , hidden: true },
        ...
        initCheckInfoValue: function(){
            var data = $("#ipranChildFlowGrid").grid("getSelection");
            if (data != null || data != undefined || data != {}){
                ...
                cloudNetWorkResCheckFlowAction.queryCheckInfo(param, function(res){
                    if (res.success){
                        ...
                        $("#upDeviceTypeA").combobox('value', res.data.A_OTHER_TYPE);
                        $("#upDeviceTypeZ").combobox('value', res.data.Z_OTHER_TYPE);
        ...
        qrycircuitInfo: function () {
            ...
            cloudNetWorkResCheckFlowAction.qrycircuitInfo(param, function(res){
                if (res.success){
                    $("#ipranChildFlowGrid").grid("reloadData", res.data);
                }else {
        ...
        submit: function () {
            var me = this;
            var circuitData = $("#ipranChildFlowGrid").grid("getCheckRows");//订单信息
        ...
        saveCheckInfo: function(){
            ...
            var resSatisfyA = $("#A_RES_SATISFY").val();
            if (resSatisfyA == null || resSatisfyA == ''){
                fish.info("A端IPRAN资源是否满足不能为空！");
                return;
            }
            var resSatisfyZ = $("#Z_RES_SATISFY").val();
            if (resSatisfyZ == null || resSatisfyZ == ''){
                fish.info("Z端IPRAN资源是否满足不能为空！");
                return;
            }
            ...
            if (resSatisfyA == '0' || resSatisfyZ == '0'){
                var resDesc = $("#resDesc").val();
                if (resDesc == null || resDesc == ''){
                    fish.info("资源情况描述不能为空！");
                    return;
                }
            }
            var params = {};
            params.circuitData = circuitData;
            var checkInfo = {};
            checkInfo.resSatisfyA = $("#A_RES_SATISFY").val();
            checkInfo.resSatisfyZ = $("#Z_RES_SATISFY").val();
            checkInfo.upDeviceTypeA = $("#upDeviceTypeA").val();
            checkInfo.upDeviceTypeZ = $("#upDeviceTypeZ").val();
