define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    ...
    return fish.View.extend({
            ...
            //添加电路按钮
             $("#addAndSelectRow").off('click').on('click',function(e) {
                ...
                if (actType!="101" && actType!="107"){
                    $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL',true);//取消非电路信息的字段校验
                }else{
                    $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL,circuitCode',true);//取消非电路信息的字段校验
                }
                var checkNull = $("#digitalCir-Form").isValid();
                if (actType!="101" && actType!="107"){
                    $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL',false);//取消非电路信息的字段校验
                }else{
                    $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL,circuitCode',false);//取消非电路信息的字段校验
                }
                if(!checkNull){
        ...
        submit:function () {
            ...
            $('#digitalCir-Form').validator('setIgnoreField', 'speed,circuitCode,A_belong_region,Z_belong_region,A_interface_type,Z_interface_type,requFineTime', true);
            var checkNull = $("#digitalCir-Form").isValid();
            $('#digitalCir-Form').validator('setIgnoreField', 'speed,circuitCode,A_belong_region,Z_belong_region,A_interface_type,Z_interface_type,requFineTime', false);
            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
