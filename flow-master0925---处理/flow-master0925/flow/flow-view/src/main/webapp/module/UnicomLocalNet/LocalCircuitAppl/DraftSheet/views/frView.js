define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    ...
    return fish.View.extend({
            ...
            //添加电路按钮
             $("#addAndSelectRow").off('click').on('click',function(e) {
                ...
                if (actType!="101" && actType!="107"){
                    $('#fr-Form').validator('setIgnoreField', 'circuitCode,CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', true);
                }else{
                    $('#fr-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', true);
                }
                var checkNull = $("#fr-Form").isValid();
                if (actType!="101" && actType!="107"){
                    $('#fr-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', false);
                }else{
                    $('#fr-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME', false);
                }
                if(!checkNull){
        ...
        submit:function () {
            ...
            $('#fr-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,A_belong_region,Z_belong_region' +
