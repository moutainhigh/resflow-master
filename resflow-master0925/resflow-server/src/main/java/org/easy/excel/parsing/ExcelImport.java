...
public class ExcelImport extends AbstractExcelResolver{
	 ...
	 * @param dataRowIndex 标题索引
	 * @return
	 * @throws Exception
	 */
	public ExcelImportResult readExcel(String id, Integer titleIndex,InputStream excelStream,Integer sheetIndex,boolean multivalidate,Integer dataRowIndex) throws Exception {
		//从注册信息中获取Bean信息
		...
		return doReadExcel(excelDefinition,titleIndex,excelStream,sheetIndex,multivalidate,dataRowIndex);
	}

	protected ExcelImportResult doReadExcel(ExcelDefinition excelDefinition,Integer titleIndex,InputStream excelStream,Integer sheetIndex,boolean multivalidate,Integer dataRowIndex) throws Exception {
		...
		//获取标题
		List<String> titles = readTitle(excelDefinition,sheet,dataRowIndex);
		//获取Bean
		List<Object> listBean = readRows(result.getErrors(),excelDefinition,titles, sheet,dataRowIndex,multivalidate);
	...
	protected <T> List<T> readRows(List<ExcelError> errors,ExcelDefinition excelDefinition, List<String> titles, Sheet sheet,int dataRowIndex,boolean multivalidate)throws Exception {
		int rowNum = sheet.getLastRowNum();
		//读取数据的总共次数
//		int totalNum = rowNum - dataRowIndex - 1;
		int startRow =  dataRowIndex+1;
		List<T> listBean = new ArrayList<T>();
		for (int i = startRow; i <= rowNum; i++) {
			Row row = sheet.getRow(i);
			if(row != null){
				Object bean = readRow(errors,excelDefinition,row,titles,i+1,multivalidate);
				listBean.add((T) bean);
			}
		}
		return listBean;
	...
	protected List<String> readTitle(ExcelDefinition excelDefinition, Sheet sheet,int dataRowIndex) {
		// 获取Excel标题数据
		Row hssfRowTitle = sheet.getRow(dataRowIndex);
