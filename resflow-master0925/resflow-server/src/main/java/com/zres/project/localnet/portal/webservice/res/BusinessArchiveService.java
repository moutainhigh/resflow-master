...
public class BusinessArchiveService implements BusinessArchiveServiceIntf {
    ...
    @Override
    public Map businessArchive(Map<String,Object> param) { //gom_bdw_srv_ord_info.SRV_ORD_ID
        logger.info("业务实例归档接口开始：" );
        String id = MapUtils.getString(param, "srvOrdId");
        ...
        try {
            ...
            if("102".equals(MapUtils.getString(resmap,"ACTIVE_TYPE",""))){
                ...
                params.put("srvOrdId",id);
                params.put("flag",BasicCode.SECOND);
...
//            rsd.saveJson(map);
        ...
        try {
            ...
            if ("1".equals(respCode)) {
                ...
                // 如果是二干本地起单，需要入库起租时间
                boolean res = BasicCode.SECONDARY.equals(MapUtils.getString(resmap,"RESOURCES"));
                if(res){
    ...
    @Override
    public Map businessArchiveLocalSche(Map<String,Object> param) { //gom_bdw_srv_ord_info.SRV_ORD_ID
        logger.info("本地调度业务实例归档接口开始：");
        String id = MapUtils.getString(param, "srvOrdId");
        String flag = MapUtils.getString(param, "flag");
        String json = "";
        String url = "";
        String zy_response = "";
        String repeatSuccess = "";
        Map retmap = new HashMap();
        Map resmap = new HashMap();
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); //设置日期格式
        String createdate = df.format(new Date());
        ...
        try {
            ...
            if("102".equals(MapUtils.getString(resmap,"ACTIVE_TYPE",""))){
                Map<String,Object> params = new HashMap<>();
                params.put("srvOrdId",id);
                params.put("flag",BasicCode.LOCAL);
                retmap = resDisassembleServiceIntf.resDisassemble(params);
                if(!MapUtils.getBoolean(retmap,"returncode")){
                    retmap.put("returncode","失败");
                    return retmap;
...
//            rsd.saveJson(map);
        ...
        //获取url
        try {
            url = rsd.queryUrl("BusinessArchive");
            map.put("url", url);
...
//            rsd.saveJson(map);
        ...
        try {
            ...
            rmap.put("attr_action", "BusinessArchive");
            rmap.put("attr_code", respCode);
            if(StringUtils.hasText(repeatSuccess)){
                rmap.put("attr_code", repeatSuccess);
            }
            rmap.put("attr_name", "");
            rmap.put("attr_value_name", "业务实例归档接口返回结果");
            rmap.put("create_date", createdate);
            ...
            logger.info("本地调度业务实例归档接口结束---" + respCode + "---" + respDesc);
        }
        catch (Exception e) {
            logger.error("接口报文解析入库异常！异常信息：" + e.getMessage(),e);
            retmap.put("returncode", "失败");
