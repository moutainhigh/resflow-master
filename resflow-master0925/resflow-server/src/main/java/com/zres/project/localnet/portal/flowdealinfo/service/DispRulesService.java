...
public class DispRulesService {
    ...
    private List<String> getCircuitAllOrderId(String orderId){
        /**
         * 1，查询是否有父流程；
         * 2，查询是否由二干下发； (本来以为是不需要的，测试发现是需要的，比如二干下发本地，会在二干生成电路调度环节就需要)
        ...
        if (!StringUtils.isEmpty(parentOrderId)){
            ...
            List<Map<String, Object>> childFlowList = orderDealDao.queryAlreadyChildFlowList(parentOrderId);
            for (Map<String, Object> childFlow : childFlowList){
                String orderIdChild = MapUtils.getString(childFlow, "ORDER_ID");
                if (!orderIdChild.equals(orderId)){
                    allOrderIdList.add(orderIdChild);
                }
            }
        }
        Map<String,Object> ifFromSecondaryMap = webServiceDao.qryGomOrderAttr(orderId, "ORDER_ID");
        ...
        if(MapUtils.isNotEmpty(ifFromSecondaryMap)){
            String orderIdSec = MapUtils.getString(ifFromSecondaryMap,"ATTR_VAL");
