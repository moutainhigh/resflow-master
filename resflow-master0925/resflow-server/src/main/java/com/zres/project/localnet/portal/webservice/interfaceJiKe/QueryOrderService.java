...
public class QueryOrderService implements QueryOrderServiceIntf {
    ...
    public Map queryOrder(@RequestBody String request) {
        ...
        try {
            ...
            String subscribeIdRela = cstOrd.getString("SUBSCRIBE_ID_RELA");
    ...
    public Map<String, Object> wrapResultMap(String tradeId,String flowId,String serialNumber) {
        ...
        List<Map<String, Object>> srvList = wsd.qrySrvOrdIdByTradeId(tradeId,flowId);
        if (!CollectionUtils.isEmpty(srvList)){
            List<Map<String, Object>> list=new ArrayList<>();
            //取第一条记录判断单据是否是并行核查单
            if ("10101342".equals(MapUtils.getString(srvList.get(0), "PS_ID")) || "10101322".equals(MapUtils.getString(srvList.get(0), "PS_ID"))){
                //并行核查单
                for (Map<String, Object> map : srvList){
                    String parallerlFlag = MapUtils.getString(map, "PARALLEL_FLAG");
                    List<Map<String, Object>> tacheInfoList = wsd.qryTacheInfo(MapUtils.getString(map, "ORDER_ID"));
                    if (!CollectionUtils.isEmpty(tacheInfoList)){
                        for(int i=0;i<tacheInfoList.size();i++){
                            Map<String, Object> tacheMap= tacheInfoList.get(i);
                            Map  TACHE_INFO=new HashMap();
                            if ("B".equals(parallerlFlag)){
                                TACHE_INFO.put("OTHER_FLAG", "02");
                            }else if ("secondA".equals(parallerlFlag)){
                                TACHE_INFO.put("OTHER_FLAG", "03");
                            }else if ("secondZ".equals(parallerlFlag)){
                                TACHE_INFO.put("OTHER_FLAG", "04");
                            }else if ("localA".equals(parallerlFlag)){
                                TACHE_INFO.put("OTHER_FLAG", "05");
                            }else if ("localZ".equals(parallerlFlag)){
                                TACHE_INFO.put("OTHER_FLAG", "06");
                            }
                ...
                String orderId = MapUtils.getString(srvList.get(0), "ORDER_ID");
                List<Map<String, Object>> tacheInfoList = wsd.qryTacheInfo(orderId);
