...
public class CheckOrderInfoService implements CheckOrderInfoIntf {
    ...
    public Map<String, Object> checkCircuitCode(Map<String, Object> params) {
        ...
        if ("101".equals(actType)) {
            ...
            Map<String, Object> map = checkOrderInfoDao.queryCircuitCodeNum(circuitCode);
            if (map != null) {
                String applyOrdId = map.get("APPLY_ORD_ID").toString();
                if ("200000004".equals(map.get("ORDER_STATE"))) {// 流程已结束
                    if (!"102".equals(map.get("ACTIVE_TYPE"))) {
                        retMap.put("success", true);
                        retMap.put("message", "电路编号校验成功！");
                    }
                    else {
                        retMap.put("success", false);
                        retMap.put("message", "电路编号：" + circuitCode + "对应的" + applyOrdId + "单子已拆单！");
                    }
                }
                else if ("200000002".equals(map.get("ORDER_STATE"))) {// 已启流程
                    retMap.put("success", false);
                    retMap.put("message", "电路编号:" + circuitCode + "对应的" + applyOrdId + " 单子正在调度中！");

                }
                else if ("200000003".equals(map.get("ORDER_STATE"))) {// 退到服开流程
                    retMap.put("success", false);
                    retMap.put("message", "电路编号:" + circuitCode + "对应的" + applyOrdId + " 单子在草稿单！");
                }
            }
            else {
                retMap.put("success", true);
                retMap.put("choice", true);
                retMap.put("message", "没有查到相关联的单子,请确认是否是存量数据");
            }
        }
        retMap.put("success", true);
        return retMap;
    ...
    public Map<String, Object> checkTradeId(Map<String, Object> params) {
        ...
        /**
         *   由于只是通过业务号码去查，所以isToBeQuery 取值为0或者1 都无所谓，资源配置都会通过业务号码做精确查找
         */
        ...
        if ("101".equals(actType)) {
            ...
            if (!MapUtils.getBooleanValue(resData, "success")){
                flag = false;
            }
...
//            }
        ...
        //非新开要验证是否在途单
            ...
            for (Map stateMap : stateList) {
                ...
                //作废或者完成才允许
                if (srv_ord_stat != null && ("10F".equals(srv_ord_stat) || "10X".equals(srv_ord_stat))) {
                    continue;
                }
                else {
                    flag = false;
    ...
    public String querySrvOrderState(Map<String, Object> params) {
        ...
        for (Map stateMap: stateList) {
            ...
            else if(srv_ord_stat!=null && "10N".equals(srv_ord_stat)){
