...
public class TimeOutJobCrontab {
    ...
    public void timeOutJob() throws Exception {
        if (alarmExecute) {
            ...
            String smsContent = "工单超时提示：【二干调度系统】收到客户【custNameChinese】的一条工单，主题为【applyOrdName】，申请单编号为【applyOrdId】已经超时timeOutHour个小时,请尽快反馈。";
                ...
                for (int i = 0; i < mapsDealTotal.size(); i++) {
                    ...
                    try {
                        ...
                        smsContentT = smsContentT.replaceAll("applyOrdName", applyOrdName)
                            .replaceAll("applyOrdId", applyOrdId)
