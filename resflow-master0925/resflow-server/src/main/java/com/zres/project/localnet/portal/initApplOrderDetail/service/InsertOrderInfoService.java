...
import com.zres.project.localnet.portal.webservice.res.BusinessCreateServiceIntf;

import com.zres.project.localnet.portal.webservice.res.ResSuspendServiceIntf;
...
public class InsertOrderInfoService implements InsertOrderInfoIntf {
    ...
    public String orderInfoSave(Map<String, Object> map) {
        ...
        int seq_gom_bdw_cst_ord = this.querySequence("GOM_BDW_CST_ORD"); //获取客户表序列值
        ...
        try {
            ...
            appliTitle = this.getAppliTitle();
            orderInfomap.put("APPLY_ORD_ID", appliTitle);
            this.insertCustomerInfo(orderInfomap); //插入客户信息
            ...
            for (int i = 0; i < cirData1.size(); i++) {
                ...
                int seq_gom_bdw_srv_ord_info = this.querySequence("GOM_BDW_SRV_ORD_INFO"); //获取订单表序列值
                this.attachSave(map, seq_gom_bdw_srv_ord_info); //保存附件信息
                this.circuitAttachSave(map, seq_gom_bdw_srv_ord_info); //保存电路信息附件
                ...
                if ("20181221006".equals(service_id)) {
                    ...
                    orderInfomap.put("SERIAL_NUMBER", MapUtils.getString(cirDatalMap,"INSTANCE_ID"));
                ...
                this.insertOrderInfo(orderInfomap); //插入定单信息
                this.packageCircuitInfo(cirDatalMap, service_id, seq_gom_bdw_srv_ord_info, currDateStr); //包装并插入电路信息
            }
        }
        catch (Exception e) {
    ...
    public String orderInfoSubmit(Map<String, Object> map) {
        ...
        try {
            ...
            if (orderInfo.containsKey("APPLY_ORD_ID") && MapUtil.getString(orderInfo, "APPLY_ORD_ID") != null) {
                result = this.orderInfoSubmInDraft(map); //草稿
            }
            else {
                result = this.orderInfoSubmDirect(map);
            }
        }
        catch (Exception e) {
            result = "error";
    ...
    public String orderInfoSubmInDraft(Map<String, Object> map) throws Exception {
        ...
        try {
            ...
            Map<String, Object> proviceOrg = orderDealDao.getProviceOrg(Long.parseLong(handleDepId));
            String provienceHandleDepId = MapUtils.getString(proviceOrg, "ORG_ID"); //归属省受理区域
           //将 CustID添加到 数据中
            ...
            String procesParam = this.getProcesParam(service_id, orderType, circuitReq);
            JSONArray cirData1 = JSON.parseArray(map.get("cirData").toString()); //获取电路信息
            ...
            for (int i = 0; i < cirData1.size(); i++) {
                ...
                if (MapUtil.getString(cirDatalMap, "SRV_ORD_ID") == null) {
                    ...
                    acceptAreaMap.put("REGION_ID", provienceHandleDepId);
                    ...
                    this.insertTrackLogInfo(orderId); //发起申请日志
                    ...
                    if ("".equals(instance_id) || instance_id == null||"101".equals(circuitReq)) {
                        orderInfomap.put("INSTANCE_ID", seq_gom_bdw_srv_ord_info); //实例id
                        cirDatalMap.put("INSTANCE_ID",seq_gom_bdw_srv_ord_info);
                    }
                    else {
                    ...
                    // 不能起线程，必须先挂起补录单，再调用创建接口
                    //   resSuspendServiceIntf.resSuspendSupplement(suspendParam);
                    suspendParam.put("isStartThread",false);
                    Map supRet= resSupplementDealServiceIntf.supplementStop(suspendParam);
                    if(!MapUtils.getBoolean(supRet,"success")){
                        TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
                        throw new Exception("补录单调用资源接口异常，异常原因：" + MapUtils.getString(supRet, "message"));
                    }
                    //TODO: 2019/3/5 匹配配置表
                    this.packageCircuitInfo(cirDatalMap, service_id, seq_gom_bdw_srv_ord_info, currDateStr); //包装并插入电路信息
                }
            ...
            if (!NeedDeleteSrvOrdIDList.isEmpty()) {
                ...
                delOrderInfoDao.delOrdAttrInfoBySrvId(NeedDeleteSrvOrdIDList);
            }
            ...
            for (int i = 0; i < cirData1.size(); i++) {
                ...
                if (servid == null || servid == 0) {
                    ...
                    if("".equals(orderInfomap.get("SRV_ORD_ID"))){
                        orderInfomap.put("SRV_ORD_ID", servid);
                    }else{
                        orderInfomap.remove("SRV_ORD_ID");
                        orderInfomap.put("SRV_ORD_ID", servid);
                    }
                    Map<String, Object> creaOrderMap = new HashMap<String, Object>();
                    creaOrderMap.put("ORDER_CONTENT", ORDER_CONTENT);
                    ...
                    if (queryOrderId != null) {
                        ...
                        String woId = insertOrderInfoDao.queryWoIdByOrderId(orderId); //查询新建申请单环节工单id
                        Map<String, Object> orderMap = new HashMap<>();
                        orderMap.put("orderId",orderId);
                        orderMap.put("reqFindate",requFineTime);
                        orderInfomap.remove("ORDER_ID");
                        orderInfomap.put("ORDER_ID", orderId);
                        webServiceDao.updateOrderReqFinDate2(orderMap);
                        reSubmit(queryOrderId,servid.toString(),woId); //退单重提
                    }
                    else {
                        Map retMap = orderDealService.createOrder(creaOrderMap);
                        ...
                        //启流程创建实例
//                        int num = orderDealDao.qryInterResult(servid.toString(),"ResBusinessCreate");
//                        if ( num < 1 ){
//                            Map<String,Object> param = new HashMap<>();
//                            param.put("srvOrdId",servid.toString());
//                            Map retmap =  businessCreateServiceIntf.businessCreate(param);
//                            if (!"成功".equals(MapUtils.getString(retmap, "returncode"))) {
//                                TransactionAspectSupport.currentTransactionStatus()
//                                        .setRollbackOnly();
//                                throw new Exception("调用资源创建接口异常，异常原因：" + MapUtils.getString(retmap, "returndec"));
//                            }
//                        }
                    }
                    /**
                    ...
                    this.attachSave(map, servid); //保存附件信息
                    this.circuitAttachSave(map, servid); //保存电路信息附件
                    delOrderInfoDao.delOrdAttrInfoBySrvIdItem(servid.toString()); //删除电路信息 后插入新的
                    this.packageCircuitInfo(cirDatalMap, service_id, servid, currDateStr); //包装并插入电路信息
                    //发送短信通知
                    /*
                    if(sendCount==0){
    ...
    public String orderInfoSubmDirect(Map<String, Object> map) throws Exception {
        ...
        try {
            ...
            if ("20181221006".equals(service_id)) {
                queryProcessMap.put("codeType", "flow_secondary_inside"); //所有局内流程 写死为内部流程
            }
            else {
                queryProcessMap.put("codeType", "flow_secondary_cust"); //流程类型 写死为内部流程
            ...
            if ("102".equals(orderType)) { //单据类型102代表核查
                queryProcessMap.put("codeType", "flow_secondary_check"); //流程类型 写死为核查流程
            ...
            Date currdate = new Date();
            orderInfomap.put("CST_ORD_ID", seq_gom_bdw_cst_ord); //客户序列
            orderInfomap.put("IS_GROUP_CUST", IS_GROUP_CUST); //是否集团直管客户
            orderInfomap.put("USER_ID", operStaffId); //发起人
            orderInfomap.put("SRV_ORD_STAT", "10N");
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            String currDateStr = sdf.format(currdate);
...
//                    int num = orderDealDao.qryInterResult(String.valueOf(seq_gom_bdw_srv_ord_info),"ResBusinessCreate");
//                    if ( num < 1 ){
//                        //启流程时调用资源创建接口
//                        Map<String,Object> param = new HashMap<>();
//                        param.put("srvOrdId",String.valueOf(seq_gom_bdw_srv_ord_info));
//                        Map retmap =  businessCreateServiceIntf.businessCreate(param);
//                        if (!"成功".equals(MapUtils.getString(retmap, "returncode"))) {
//                            TransactionAspectSupport.currentTransactionStatus()
//                                    .setRollbackOnly();
...
//                    }
    ...
    public String getAppliTitle1() {
        int applTitle = this.querySequence("applTitle");
...
//        scheduNumMap.put("userId", userId);
    ...
    public synchronized String getAppliTitle() {
        String userId = ThreadLocalInfoHolder.getLoginUser().getUserId(); //获取当前用户
//        Map<String, Object> orgMap = insertOrderInfoDao.queryOrgIdSecond(userId);
//        String parent_org_id = MapUtil.getString(orgMap, "PARENT_ORG_ID");
//        String org_id = MapUtil.getString(orgMap, "ORG_ID");
//
//        if(parent_org_id!=null && "1".equals(parent_org_id)) {
//            return insertOrderInfoDao.queryScheduNumFunction(org_id);
//        }
    ...
    public void packageCircuitInfo(Map<String, Object> cirDatalMap, String service_id, int srv_ord_id, String currDateStr) throws Exception {
        ...
        this.querySequence("gom_BDW_srv_ord_attr_info"); //序列刷掉一个，解决第一次提交报违反唯一约束
        this.insertordAttrInfo(cirInfoArray); //插入电路信息
    }
     ...
     * @return void
     */
    public void reSubmit(String orderId ,String servid, String woId){
        ...
        flowAction.complateWo("-2000", woChildDTO); //回单

        //修改申请单状态
        orderDealDao.updateSrvOrdState(servid,"10N");
        ...
        String trackMessage = "[" + operStaffName + "将工单单号：" + woId + "][重发]";
        operLogMap.put("trackMessage", trackMessage);
        operLogMap.put("trackContent", "[退单重发]");
        operLogMap.put("operType", OrderTrackOperType.OPER_TYPE_16);
