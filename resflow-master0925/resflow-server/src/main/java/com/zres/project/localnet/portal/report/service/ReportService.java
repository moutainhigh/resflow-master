...
public class ReportService implements ReportIntf {
     ...
     */
    @Override
    public Map<String, Object> queryDisOrderList(Map<String, Object> map) {
        ...
        //得到总数
        int rowCount = reportDao.countDisOrder(params);
        if (rowCount > 0) {
            //得到分页数据
            List<Map<String, Object>> mapList = reportDao.queryDisOrderList(params);
    ...
    public Map<String, Object> queryCircuitInfoForBusiReport(Map<String, Object> param) {
        Map<String, Object> resMap = new HashMap<>();
        try{
            ...
            List<Map<String, Object>> circuitList = reportDao.queryCircuitInfoForBusiReport(param);
            if (circuitList != null){
                //封装分页数据
                Map<String, Object> pageMap = new HashMap<>();
                pageMap.put("page", MapUtils.getObject(param,"pageIndex"));
                pageMap.put("rows", circuitList);
                pageMap.put("records", reportDao.queryCountForBusiReport(param));
                resMap.put("success", true);
                resMap.put("data", pageMap);
                resMap.put("msg", "查询成功！");
            }
        }catch(Exception e){
            logger.info(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>业务报表-电路明细表查询异常：" + e.getMessage());
            resMap.put("success", false);
            resMap.put("msg", e.getMessage());
        }
        return resMap;
