define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    'text!module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/templates/checkView.html',
...
], function(digitalCircutAction,digitalCircuitView,i18n,css) {
    var $grid;
    var productType;
    var productTypeText;
    var actType;
    ...
    return fish.View.extend({
        ...
        template: fish.compile(digitalCircuitView),
        ...
        initGred:function (mydata) {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    {name: 'speed',label: '带宽',width: 100,sorttype: "string",align:"center",hidden:true},
                    {name: 'speedName',label: '带宽',width: 100,sorttype: "string",align:"center",},
                    {name: 'IPv4',label: '客户申请IPv4地址数量',width: 100,sorttype: "string",align:"center",},
                    {name: 'IPv6',label: 'IPv6前缀',width: 100,sorttype: "string",align:"center",},
                    ...
                    {name: 'A_interface_type', label: 'A端接口类型', width: 1,sorttype: "string",align:"center",hidden:true},
                    ...
                    {name: 'A_customer_manager_tel', label: 'A端客户经理联系电话', width: 1,sorttype: "string",align:"center",hidden:true},
                    ...
                    {name: 'Z_interface_type', label: 'Z端接口类型', width: 1,sorttype: "string",align:"center",hidden:true},
                    ...
                    {name: 'Z_customer_manager_tel', label: 'Z端客户经理联系电话', width: 1,sorttype: "string",align:"center",hidden:true},
                ]
            };
            ...
            $grid.on('click', '.js-delete', function() {
                ...
                var popup = fish.popupView({
                    url: 'module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/views/updateCheckView',
                    width: "95%",
                    height:"90%",
                    modal:true,
                    canClose:true,
        ...
        if (actType!="101" && actType!="107"){
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME,HANDLE_DEP',true);//取消非电路信息的字段校验
        }else{
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL,circuitCode,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME,HANDLE_DEP',true);//取消非电路信息的字段校验
        }
        var checkNull = $("#digitalCir-Form").isValid();
        if (actType!="101" && actType!="107"){
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME,HANDLE_DEP',false);//取消非电路信息的字段校验
        }else{
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_CITY,CUST_CONTACT_MAN_NAME,CUST_TEL,circuitCode,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME,HANDLE_DEP',false);//取消非电路信息的字段校验
        }
        if(!checkNull){
        ...
        digitalCircutAction.checkTradeId({tradeId:$("#tradeId").val(),actType:actType,orderType:orderType.value},function (res) {
            ...
            var circuitInfo = {};
            var circuitCodeArea =['tradeId','serviceId','circuitReq','circuitCode','oldCircuitCode','speed','requFineTime','accepTime','createTime','slaFlag', 'slaServOpen', 'slaNetQuAss', 'slaSaleServ', 'proPriDeg', 'cirLeaseRange', 'cirUse', 'serviceName','circuitReqName','speedName','cir_remark','A_belong_province','A_belong_city','A_belong_county','A_belong_region','A_customer_name','A_installed_add','A_contact_man','A_contact_tel','A_interface_type','A_old_belong_region','A_old_belong_county','A_old_installed_add','A_customer_manager','A_customer_manager_tel','Z_belong_province','Z_belong_city','Z_belong_county','Z_belong_region','Z_customer_name','Z_installed_add','Z_contact_man','Z_contact_tel','Z_interface_type','Z_old_belong_region','Z_old_belong_county','Z_old_installed_add','Z_customer_manager','Z_customer_manager_tel','A_belong_region_id','Z_belong_region_id','A_old_belong_region_id','Z_old_belong_region_id','INSTANCE_ID','IPv4','IPv6','A_belong_city_id','Z_belong_city_id'];
        ...
        initDigPage:function () {
            ...
            $("#applicat-Page-Title").html("核查 - "+productTypeText+"-"+actTypeText);
        ...
        submit:function () {
            ...
            $('#digitalCir-Form').validator('setIgnoreField', 'speed,circuitCode,slaFlag,A_interface_type,Z_interface_type,requFineTime', true);
            var checkNull = $("#digitalCir-Form").isValid();
            $('#digitalCir-Form').validator('setIgnoreField', 'speed,circuitCode,slaFlag,A_interface_type,Z_interface_type,requFineTime', false);
            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
       ...
       //     orderInfoObj.itemType = 'check';
            orderInfoObj.CUST_ID =custOrdId;

            var _this =this;
            $("#digitalCir-Form").blockUI({message: '请稍后...'}).data('blockui-content', true);
            ...
            digitalCircutAction.orderInfoSubmit(orderInfoObj,function(data) {
                ...
                if(arrdata[0]=='success'){
                    ...
                    // fish.toast("success","");
                    $("#digitalCir-Form").unblockUI().data('blockui-content', false);
                    _this.popup.close();
                }else{
                    $("#digitalCir-Form").unblockUI().data('blockui-content', false);
        ...
        save:function () {
            ...
            orderInfoObj.upLoadResult=upLoadResult;
            var _this =this;
            $("#digitalCir-Form").blockUI({message: '保存中'}).data('blockui-content', true);
