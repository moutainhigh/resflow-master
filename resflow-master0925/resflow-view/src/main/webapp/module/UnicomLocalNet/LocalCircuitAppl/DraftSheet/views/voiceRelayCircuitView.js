define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    'text!module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/templates/voiceRelayCircuitView.html',
...
], function(digitalCircutAction,voiceRelayCircuitView,i18n,css) {
    var $grid;
    var productType;
    var productTypeText;
    ...
    return fish.View.extend({
        template: fish.compile(voiceRelayCircuitView),
        ...
        initGred:function (mydata) {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    {name: 'AConvServiceNum', label: 'A端汇聚电路业务号码',  width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'ZConvServiceNum', label: 'Z端汇聚电路业务号码',  width: 1,sorttype: "string",align:"center",hidden:true},
                    ...
                    {name: 'A_customer_equip', label: 'A端客户设备情况', width: 1,sorttype: "string",align:"center",hidden:true},
                    ...
                    {name: 'Z_customer_equip', label: 'Z端客户设备情况', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'operationAfter',label: '操作',align:"center",width: 200,sorttype: "string",
                        formatter: function(cellval, opts, rwdat, _act) {
                            if (flag  == "view"){
                                return '<div class="btn-group">' +
                                    '<button type="button" class="btn btn-link js-view " title="查看"><span class="glyphicon glyphicon glyphicon-edit" style="font-size:15px;color: #eb4a4b"></span></button>' +
                                    '</div>'
                            }else{
                                return '<div class="btn-group">' +
            ...
            $grid.on('click', '.js-delete', function() {
                ...
                var popup = fish.popupView({
                    url: 'module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/views/updateVoiceRelayCircuitView',
                    width: "95%",
                    height:"90%",
                    modal:true,
                    canClose:true,
            ...
            $("#addAndSelectRow").click(function() {
                ...
                if (actType!="101" && actType!="107"){
                    $('#voiceRelayCircuit-Form').validator('setIgnoreField', 'CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL', true);
                }else{
                    $('#voiceRelayCircuit-Form').validator('setIgnoreField', 'CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL', true);
                }
                var checkNull = $("#voiceRelayCircuit-Form").isValid();
                if (actType!="101" && actType!="107"){
                    $('#voiceRelayCircuit-Form').validator('setIgnoreField', 'CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL', false);
                }else{
                    $('#voiceRelayCircuit-Form').validator('setIgnoreField', 'CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL', false);
                }
                if(!checkNull){
                ...
                digitalCircutAction.checkTradeId({tradeId:$("#tradeId").val(),serialNumber:$("#serialNumber").val(),actType:actType,orderType:orderType.value},function (res) {
                    ...
                    digitalCircutAction.checkCircuitCode(param, function(res){
                        if(res.success){
                            ...
                            var circuitInfo = {};
                            var circuitCodeArea =['tradeId','serialNumber','serviceId','circuitReq','circuitCode','oldCircuitCode','speed','requFineTime','accepTime','createTime','slaFlag', 'slaServOpen', 'slaNetQuAss', 'slaSaleServ', 'proPriDeg', 'cirLeaseRange', 'cirUse', 'serviceName','circuitReqName','speedName','cir_remark', 'AConvServiceNum', 'ZConvServiceNum', 'A_belong_province', 'A_belong_city', 'A_belong_county', 'A_belong_region', 'A_customer_name', 'A_installed_add', 'A_contact_man', 'A_contact_tel', 'A_customer_manager', 'A_customer_manager_tel', 'A_interface_type', 'A_old_belong_region', 'A_old_belong_county', 'A_old_installed_add', 'A_customer_equip', 'Z_belong_province', 'Z_belong_city', 'Z_belong_county', 'Z_belong_region', 'Z_customer_name', 'Z_installed_add', 'Z_contact_man', 'Z_contact_tel', 'Z_customer_manager', 'Z_customer_manager_tel', 'Z_interface_type', 'Z_old_belong_region', 'Z_old_belong_county', 'Z_old_installed_add', 'Z_customer_equip','A_belong_region_id','Z_belong_region_id','A_old_belong_region_id','Z_old_belong_region_id','INSTANCE_ID','A_belong_city_id','Z_belong_city_id'];
                            ...
                            for(var i=0;i<circuitCodeArea.length;i++){
                                ...
                                if("A_belong_region_id"==circuitCodeArea[i]){
                                    var key = circuitCodeArea[i];
                                    if($("#A_belong_region").popedit('getValue')!=null){
                                ...
                                if("Z_belong_region_id"==circuitCodeArea[i]){
                                    var key = circuitCodeArea[i];
                                    if($("#Z_belong_region").popedit('getValue')!=null){
        ...
        initPageParam:function () {
            ...
            if(actType=="101"){
                ...
                $("#A_old_installed_add_div").hide();
                $("#Z_old_installed_add_div").hide();
            };
        },
        ...
        querySelectInf :function(res){
            alert(res);
            //var initGred = $.proxy(this.initGred,this); //函数作用域改变
            digitalCircutAction.querySelectInfo(res, function(data){
                // 修改页面
                var circuitData = data.circuitInfo;
                alert(circuitData);
            })
        ...
        queryWorkOrders: function(page, rowNum, sortname, sortorder) {
            ...
            //获取表单信息
            var formValue = $('#voiceRelayCircuit-Form').form("value");
        ...
        submit:function () {
            ...
            $('#voiceRelayCircuit-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,A_belong_region,Z_belong_region' +
                ',speed,cirLeaseRange,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man,A_contact_tel' +
                ',Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,Z_installed_add,requFineTime', true);
            var checkNull = $("#voiceRelayCircuit-Form").isValid();
            $('#voiceRelayCircuit-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,A_belong_region,Z_belong_region' +
                ',speed,cirLeaseRange,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man,A_contact_tel' +
                ',Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,Z_installed_add,requFineTime', false);
            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
            ...
            var _this =this;
            $("#voiceRelayCircuit-Form").blockUI({message: '请稍后...'}).data('blockui-content', true);
            ...
            digitalCircutAction.orderInfoSubmit(orderInfoObj,function(data) {
                ...
                if(arrdata[0]=='success'){
                    ...
                    // fish.toast("success","");
                    $("#voiceRelayCircuit-Form").unblockUI().data('blockui-content', false);
                    _this.popup.close();
                }else{
                    $("#voiceRelayCircuit-Form").unblockUI().data('blockui-content', false);
        ...
        save:function () {
            ...
            var _this =this;
            $("#voiceRelayCircuit-Form").blockUI({message: '保存中'}).data('blockui-content', true);
