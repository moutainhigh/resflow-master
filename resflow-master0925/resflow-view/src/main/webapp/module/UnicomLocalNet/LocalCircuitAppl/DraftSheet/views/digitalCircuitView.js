define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    'text!module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/templates/digitalCircuitView.html',
    ...
    var flag;
    var fileList = null,
        maxFileSize = 50*1024*1024,                //文件大小限制5M
    ...
    return ngc.View.extend({
        ...
        template: ngc.compile(digitalCircuitView),
        ...
        initGred:function (mydata) {
            ...
            $grid.on('click', '.js-delete', function() {
                ...
                var popup = fish.popupView({
                    url: 'module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/views/updateCircuitView',
                    width: "95%",
                    height:"90%",
                    modal:true,
                    canClose:true,
        ...
        if (actType!="101" && actType!="107"){
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL,CUST_MANAGER,CUST_MANAGER_TEL',true);//取消非电路信息的字段校验
        }else{
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL,CUST_MANAGER,CUST_MANAGER_TEL',true);//取消非电路信息的字段校验
        }
        var checkNull = $("#digitalCir-Form").isValid();
        if (actType!="101" && actType!="107"){
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL,CUST_MANAGER,CUST_MANAGER_TEL',false);//取消非电路信息的字段校验
        }else{
            $('#digitalCir-Form').validator('setIgnoreField','CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_TEL,CUST_MANAGER,CUST_MANAGER_TEL',false);//取消非电路信息的字段校验
        }
        if(!checkNull){
        ...
        /**
         非新开时，电路编号与原电路编号保持一致，并且电路编号需要在数据库已开通的电路中能查到
         */
        ...
        initPageParam:function () {
            ...
            if(actType=="101"){
                ...
                $("#A_OldBelongRegion").hide();
                $("#Z_OldBelongRegion").hide();
                $("#A_OldBelongCounty").hide();
                $("#A_OldInstalledAdd").hide();
                $("#Z_OldBelongCounty").hide();
                $("#Z_OldInstalledAdd").hide();
                $("#old_Circuit_Code").hide();
                $("#RELE_CREATE_APPLICATION_DIV").hide();
            };
            //业务号码非新开电路只读，选择电路后带出业务号码
            /*if(actType!="101"){
                $('#serialNumber').attr('readonly','readonly');
                $('#serialNumber').click(function (e) {
                    fish.toast('info','请选择电路');
                    $('#circuitCode').focus();
                })
            }
*/
        ...
        querySelectedInf :function(CustId){
            ...
            /*if(flag=='edit'){
                if(this.options.itemTypeStart!='102'){
                    params.cgFlag = '10C';
                }
            }*/
            var initGred = $.proxy(this.initGred,this); //函数作用域改变
        ...
        submit:function () {
            ...
            $('#digitalCir-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag,cirLeaseRange,cirUse' +
        ...
        UpdateSave : function(){
            ...
            $('#digitalCir-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag,cirLeaseRange,cirUse' +
                ',speed,cirLeaseRange,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man' +
                ',A_contact_tel,Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,Z_installed_add,requFineTime', false);            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
            }
