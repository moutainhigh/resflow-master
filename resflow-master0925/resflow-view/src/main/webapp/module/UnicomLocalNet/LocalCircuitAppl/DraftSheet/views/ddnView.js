define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    'text!module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/templates/ddnView.html',
...
], function(digitalCircutAction,ddnView,i18n,css) {
    var $grid;
    var productType;
    var productTypeText;
    ...
    return fish.View.extend({
            ...
            /*$("#HANDLE_CITY").popedit({
                ...
                open:function(e) {
                    ...
                    var options = {
                        ...
                        url: 'module/UnicomLocalNet/LocalCircuitAppl/AreaCombotree/views/acceptAreaView',
                        height: 400,
                        width: 300,
                        modal: false,
                        //draggable: false,
                        ...
                        callback: function (popup, view) {
                            popup.result.then(function (res) {
                                ...
                                res.forEach(function(val,i){
                                    if(!val.isParent){
                                        if(orgIds==''){
                                            orgNames = val.ORG_NAME ;
                                            orgIds = val.id;
                                        } else {
                                            orgNames = orgNames + ',' + val.ORG_NAME ;
        ...
        initGred:function (mydata) {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    {name: 'ddn_bussiness_type', label: 'DDN业务种类', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'SingleAndDoubleToPVC', label: '单双向PVC', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'VPorVC', label: 'VP/VC', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'AZRateSCR', label: 'AZ速率(SCR)', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'ZARateSCR', label: 'ZA速率(SCR)', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'AZRatePCR', label: 'AZ速率(PCR)', width: 1,sorttype: "string",align:"center",hidden:true},
                    {name: 'ZARatePCR', label: 'ZA速率(PCR)', width: 1,sorttype: "string",align:"center",hidden:true},
            ...
            $grid.on('click', '.js-delete', function() {
                ...
                var popup = fish.popupView({
                    url: 'module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/views/updateDdnView',
                    width: "95%",
                    height:"90%",
                    modal:true,
                    canClose:true,
        ...
        if (actType!="101" && actType!="107"){
            $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', true);
        }else{
            $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', true);
        }
        var checkNull = $("#ddn-Form").isValid();
        if (actType!="101" && actType!="107"){
            $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', false);
        }else{
            $('#ddn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', false);
        }
        if(!checkNull){
        ...
        orderType=me.options.itemTypeStart;
        ...
        digitalCircutAction.checkTradeId({tradeId:$("#tradeId").val(),serialNumber:$("#serialNumber").val(),actType:actType,orderType:orderType.value},function (res) {
            ...
            digitalCircutAction.checkCircuitCode(param, function(res){
                if(res.success){
                    ...
                    var circuitInfo = {};
                    var circuitCodeArea =['tradeId','serialNumber','serviceId','circuitReq','acceptArea','acceptAreaID','circuitCode','oldCircuitCode','speed','requFineTime','accepTime','createTime','slaFlag', 'slaServOpen', 'slaNetQuAss', 'slaSaleServ', 'proPriDeg', 'cirLeaseRange', 'cirUse', "serviceName","circuitReqName","speedName", 'cir_remark', 'ddn_bussiness_type', 'SingleAndDoubleToPVC', 'VPorVC', 'AZRateSCR', 'ZARateSCR', 'AZRatePCR', 'ZARatePCR', 'A_belong_province', 'A_belong_city', 'A_belong_county', 'A_belong_region', 'A_customer_name', 'A_installed_add', 'A_contact_man', 'A_contact_tel', 'A_customer_manager', 'A_customer_manager_tel', 'A_interface_type', 'A_old_belong_region', 'A_old_belong_county', 'A_old_installed_add', 'Z_belong_province', 'Z_belong_city', 'Z_belong_county', 'Z_belong_region', 'Z_customer_name', 'Z_installed_add', 'Z_contact_man', 'Z_contact_tel', 'Z_customer_manager', 'Z_customer_manager_tel', 'Z_interface_type', 'Z_old_belong_region', 'Z_old_belong_county', 'Z_old_installed_add','A_belong_region_id','Z_belong_region_id','A_old_belong_region_id','Z_old_belong_region_id','INSTANCE_ID','A_belong_city_id','Z_belong_city_id'];
                    ...
                    for(var i=0;i<circuitCodeArea.length;i++){
                        ...
                        if("acceptAreaID"==circuitCodeArea[i]){
                            var key = circuitCodeArea[i];
                            if($("#acceptArea").popedit('getValue').value!=undefined){
                                var value = $("#acceptArea").popedit('getValue').value;
                                circuitInfo[""+key]=""+value;
                            }
                        }
                    ...
                    $("#acceptArea").popedit('clear'); //清空popedit
                    var date = new Date();
                    $("#createTime").datetimepicker("value", date,"disable");
                    //根据登陆人初始化默认省
                    digitalCircutAction.queryProvienceTree(function (data){
        ...
        queryWorkOrders: function(page, rowNum, sortname, sortorder) {
            ...
            //获取表单信息
            var formValue = $('#ddn-Form').form("value");
        ...
        submit:function () {
            ...
            $('#ddn-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag' +
                ',speed,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man,A_contact_tel' +
                ',Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,cirLeaseRange,Z_installed_add,requFineTime', true);
            var checkNull = $("#ddn-Form").isValid();
            $('#ddn-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag' +
                ',speed,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man,A_contact_tel' +
                ',Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,cirLeaseRange,Z_installed_add,requFineTime', false);
            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
            ...
            var _this =this;
            $("#ddn-Form").blockUI({message: '请稍后...'}).data('blockui-content', true);
            ...
            digitalCircutAction.orderInfoSubmit(orderInfoObj,function(data) {
                ...
                if(arrdata[0]=='success'){
                    ...
                    // fish.toast("success","");
                    $("#ddn-Form").unblockUI().data('blockui-content', false);
                    _this.popup.close();
                }else{
                    $("#ddn-Form").unblockUI().data('blockui-content', false);
        ...
        save:function () {
            ...
            $('#ddn-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag' +
                ',speed,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man,A_contact_tel' +
                ',Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,cirLeaseRange,Z_installed_add,requFineTime', false);            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
            }
            ...
            var _this =this;
            $("#ddn-Form").blockUI({message: '保存中'}).data('blockui-content', true);
            ...
            $('.js-select-file').fileupload({
                ...
                add: function (e, data) {
                  ...
                  /*  if (fileList === null) {
                    ...
                    $('.file-remove').off('click').on('click', function (e) {
                        var name = $(e.currentTarget).prev().children(".fileName").text(), pos;
