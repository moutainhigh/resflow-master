define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    ...
    return fish.View.extend({
            ...
            for(var i =0;i<menu.length;i++){
                ...
                if(enumType=='10000506'){
                    ...
                    digitalCircutAction.queryEnum(enumType, function (data) {
                        ...
                        $("#isnetmanage").combobox('value', 1);
                        $("#EnableMulticast").combobox('value', 1);
                        $("#isUseInInternet").combobox('value', 1);
        ...
        initGred:function (mydata) {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    {name: 'BILLID', label: '计费ID',  width: 100,sorttype: "string",align:"center"},
                    ...
                    {name: 'Z_belong_city', label: 'PE端归属地市',  width: 1,sorttype: "string",align:"center",hidden:true},
                    ...
                    {name: 'Z_installed_add', label: 'CE端安装地址',  width: 1,sorttype: "string",align:"center",hidden:false},
                    ...
                    {name: 'Z_old_installed_add', label: 'CE端原安装地址',  width: 1,sorttype: "string",align:"center",hidden:true},
        ...
        if (actType!="101" && actType!="107"){
            $('#mplsVpn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME ', true);
        }else{
            $('#mplsVpn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME ', true);
        }
        var checkNull = $("#mplsVpn-Form").isValid();
        if (actType!="101" && actType!="107"){
            $('#mplsVpn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME ', false);
        }else{
            $('#mplsVpn-Form').validator('setIgnoreField', 'CUST_TEL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME ', false);
        }
        if(!checkNull){
        ...
        digitalCircutAction.checkTradeId({tradeId:$("#tradeId").val(),serialNumber:$("#serialNumber").val(),actType:actType,orderType:orderType.value},function (res) {
            ...
            digitalCircutAction.checkCircuitCode(param, function(res){
                if(res.success){
                    ...
                    var circuitInfo = {};
                    var circuitCodeArea =['tradeId','serialNumber','speedName','serviceId','circuitReq','circuitCode','oldCircuitCode','requFineTime','accepTime','createTime','slaFlag', 'slaServOpen', 'slaNetQuAss', 'slaSaleServ', 'proPriDeg', 'cirLeaseRange','serviceName','circuitReqName','BILLID', 'cir_remark', 'qos', 'qosTpye', 'isUseInInternet', 'ExistCircuitNum', 'ExistServNum', 'isGradeFlowMarkObeyCU', 'routing_protocol', 'intranetAddrOrMask', 'AsNumbe', 'EnableMulticast', 'isnetmanage', 'IPAddrType', 'PE_BelongProv', 'Z_belong_city', 'PE_BelongCounty', 'PE_BelongRegin', 'PE_PortRange', 'PE_PortRate', 'PE_Addr', 'PE_interfaceType', 'PE_IPV4Mask', 'PE_IPV6Mask', 'PE_oldBelongRegin', 'PE_oldBelongCounty', 'PE_oldAddr', 'CE_BelongProv', 'A_belong_city', 'CE_BelongCounty', 'CE_BelongRegin', 'CE_CustName', 'Z_installed_add', 'CE_interfaceType', 'CE_IPV4Mask', 'CE_IPV6Mask', 'speed', 'AccessCircuitType', 'AccessOperator', 'CE_oldBelongRegin', 'CE_oldBelongCounty', 'A_old_installed_add','PE_BelongRegin_id','PE_oldBelongRegin_id','CE_BelongRegin_id','CE_oldBelongRegin_id','INSTANCE_ID','A_belong_city_id','Z_belong_city_id'];
                    ...
                    for(var i=0;i<circuitCodeArea.length;i++){
                        ...
                        else if("A_belong_city_id"==circuitCodeArea[i]){
                            var key = circuitCodeArea[i];
                            if($("#A_belong_city").val()!=""){
                        ...
                        else if("Z_belong_city_id"==circuitCodeArea[i]){
                            var key = circuitCodeArea[i];
                            if($("#Z_belong_city").val()!=""){
        ...
        submit:function () {
            ...
            $('#mplsVpn-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,speed,slaFlag' +
                ',CE_interfaceType,PE_interfaceType,cirLeaseRange,PE_BelongProv,CE_BelongProv,A_belong_city,Z_belong_city,Z_installed_add,requFineTime', true);
            var checkNull = $("#mplsVpn-Form").isValid();
            $('#mplsVpn-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,speed,slaFlag' +
                ',CE_interfaceType,PE_interfaceType,cirLeaseRange,PE_BelongProv,CE_BelongProv,A_belong_city,Z_belong_city,Z_installed_add,requFineTime', false);
            if(!checkNull){
                // fish.toast("success","");
                // debugger;
            ...
            if (circuitData.length > 0){
                for(var i= 0; i < circuitData.length; i++){
                    ...
                    for (var key in circuitMap){
                        if (key == 'tradeId'){
                        ...
                        else if (key == 'A_old_belong_region'){
                            param.PE_oldBelongRegin = circuitMap[key];
                        }
                        else if (key == 'A_old_belong_county'){
                            param.PE_oldBelongCounty = circuitMap[key];
                        }
                        else if (key == 'A_old_installed_add'){
                            ...
                            param.Z_installed_add = circuitMap[key];
                        }else if (key == 'Z_old_belong_region'){
                            param.CE_oldBelongRegin = circuitMap[key];
                        }else if (key == 'Z_old_belong_county'){
