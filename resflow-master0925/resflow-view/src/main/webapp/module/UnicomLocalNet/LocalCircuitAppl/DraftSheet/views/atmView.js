define(['module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/action/digitalCircutAction.js',
    'text!module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/templates/atmView.html',
...
], function(digitalCircutAction,atmView,i18n,css) {
    var $grid;
    var productType;
    var productTypeText;
    ...
    return fish.View.extend({
        ...
        initGred:function (mydata) {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    {name: 'atm_bussiness_type', label: 'ATM业务种类', width: 1,sorttype: "string",align:"center",hidden:true},
            ...
            $grid.on('click', '.js-delete', function() {
                ...
                var popup = fish.popupView({
                    url: 'module/UnicomLocalNet/LocalCircuitAppl/DraftSheet/views/updateAtmView',
                    width: "95%",
                    height:"90%",
                    modal:true,
                    canClose:true,
        ...
        if (actType!="101" && actType!="107"){
            $('#atm-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', true);
        } else{
            $('#atm-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', true);
        }
        var checkNull = $("#atm-Form").isValid();
        if (actType!="101" && actType!="107"){
            $('#atm-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', false);
        } else{
            $('#atm-Form').validator('setIgnoreField', 'CUST_TEL,CUST_EMAIL,CUST_NAME_CHINESE,circuitCode,HANDLE_CITY,HANDLE_MAN_NAME,HANDLE_MAN_TEL,HANDLE_DEP,CUST_CONTACT_MAN_NAME,CUST_MANAGER,CUST_MANAGER_TEL,HANDLE_TIME', false);
        }
        if(!checkNull){
        ...
        queryWorkOrders: function(page, rowNum, sortname, sortorder) {
            ...
            //获取表单信息
            var formValue = $('#atm-Form').form("value");
        ...
        submit:function () {
            ...
            $('#atm-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag' +
                ',speed,cirLeaseRange,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man' +
                ',A_contact_tel,Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,Z_installed_add,requFineTime', true);
            var checkNull = $("#atm-Form").isValid();
            //校验结束后,将以下字段设置为必填，以便其它功能使用
            $('#atm-Form').validator('setIgnoreField', 'tradeId,serialNumber,circuitCode,slaFlag' +
                ',speed,cirLeaseRange,cirUse,A_belong_province,A_belong_city,Z_belong_province,Z_belong_city,A_installed_add,A_contact_man' +
                ',A_contact_tel,Z_contact_man,Z_contact_tel,A_interface_type,Z_interface_type,Z_installed_add,requFineTime', false);
            if(!checkNull){
                // fish.toast("success","");
                fish.error({title:'提示',message:'请检查必填数据或数据格式'})
                return;
            ...
            var _this =this;
            $("#atm-Form").blockUI({message: '请稍后...'}).data('blockui-content', true);
            ...
            digitalCircutAction.orderInfoSubmit(orderInfoObj,function(data) {
                ...
                if(arrdata[0]=='success'){
                    ...
                    // fish.toast("success","");
                    $("#atm-Form").unblockUI().data('blockui-content', false);
                    _this.popup.close();
                }else{
                    $("#atm-Form").unblockUI().data('blockui-content', false);
        ...
        save:function () {
            ...
            var _this =this;
            $("#atm-Form").blockUI({message: '保存中'}).data('blockui-content', true);
