define(['module/UnicomLocalNet/resmaster/portal/orderTacheDealView/action/operOrderAction',
    ...
    'text!module/UnicomLocalNet/resmaster/portal/orderTacheDealView/templates/dataMakeRollBackView.html',
...
], function(operOrderAction,orderRollBackAction,dataMakeRollBackView,orderStandbyAction,i18n,css) {
    return fish.View.extend({
        template: fish.compile(dataMakeRollBackView),
        ...
        submit:function () {
            ...
            if (IFHASRES) {
                ...
                toTaches = '510101041,510101081';
            }else {
                toTaches = '510101040,510101080';
            }
        ...
        circuitInfo : function() {
            ...
            $("#circuitGrid").grid({
                ...
                onCellSelect:function(e, rowid, iCol, cellcontent){ //选中单元格事件
                    ...
                    //查询需要退单的资源分配专业
                    me.qryspecialtyData(data.ORDER_ID, data.SPECIALTY_CODE);
        ...
        qrycircuitInfo : function(){
            ...
            operOrderAction.qrySrvOrdList(param,function (res) {
                if(res.flag == 1){
                    ...
                    //查询需要退单的资源分配专业
                    me.qryspecialtyData(res.data[0].ORDER_ID, res.data[0].SPECIALTY_CODE);
        ...
        //数据制作退单专业
        qryspecialtyData : function(orderId, specialty){
            var me = this;
            var params = {};
            params.orderId = orderId;
            orderRollBackAction.qrySecondSourceDispatch(params,function (res) {
                var divStr = "";
                IFHASRES = res.ifHas;
                if(res.ifHas){
                    var toSpecialty = '';
                    $.each(res.data,function(i,val){
                        divStr = divStr + '<label class="checkbox-inline">'+
                            ...
                            + val.PUB_DATE_NAME
                            +'</label>';
                        if(specialty == val.SPECIALTY_CODE){
                            toSpecialty = val.SPECIALTY_CODE;
                        }
                    });
                    me.$("#backSpecialtyDataDiv").html("");
                    me.$("#backSpecialtyDataDiv").append(divStr);
                    if (toSpecialty != '') {
                        $('#' + toSpecialty).attr('checked', true);
                    }
                }else {
                    $("#specialtyDataDiv").hide();
