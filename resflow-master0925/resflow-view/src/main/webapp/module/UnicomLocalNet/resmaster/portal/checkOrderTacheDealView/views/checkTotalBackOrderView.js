define(['module/UnicomLocalNet/resmaster/portal/orderTacheDealView/action/operOrderAction',
    ...
    'text!module/UnicomLocalNet/resmaster/portal/checkOrderTacheDealView/templates/checkTotalBackOrderView.html',
    ...
    return fish.View.extend({
        ...
        qrySpecialData : function(orderId){
            ...
            operOrderAction.qrySpecialData(orderId + '',function (res) {
                if(res.success){
                    ...
                    if (toLocalFlag){
                        me.initQryToLocalCheckData(orderId);
                    }
                }else {
        ...
        initQryToLocalCheckData : function(orderId) {
            var me = this;
            $('#backSpecialDataDiv').bind('click',function(){
                var specialFlag = $("input[name='special']:checked").val();
                if (specialFlag == '510101065') {
                    $("#localCheckAreaDiv").show();
                    operOrderAction.qryToLocalCheckData(orderId + '',function (res) {
                        if(res.success){
                            var divStr = '';
                            $.each(res.specialData,function(i,val){
                                divStr = divStr + '<label class="checkbox-inline">'+
                                    '<input type="checkbox" name="area" id="' + val.ORDER_ID
                                    + '" value= "' + val.ORDER_ID +'">'
                                    + val.DEPT_NAME
                                    +'</label>';
                            });
                            me.$("#localCheckAreaDataDiv").html("");
                            me.$("#localCheckAreaDataDiv").append(divStr);
                        }else {
                            fish.toast('error', res.message);
                        }
                    })
                }else {
                    $("#localCheckAreaDiv").hide();
                }
            });
        },
        ...
        submit:function () {
            ...
            var toSpecialTache = $("input[name='special']:checked").val();
            var areaArray = [];
            if (toSpecialTache == '510101065'){
                $("input[name='area']:checked").each(function(i){
                    areaArray.push($(this).val());
                });
                if (areaArray.length == 0){
                    fish.error({title:'提示',message:'退单本地网核查区域，至少选择一个。。。'});
                    return;
                }
            ...
            params.toSpecialTache = toSpecialTache; //退单专业
