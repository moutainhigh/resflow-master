...
define(['module/UnicomLocalNet/resmaster/portal/flowChart/action/flowChartViewAction',
    ...
    return fish.View.extend({
        ...
        dataSource: "",
        systemSource: "",
        template: fish.compile(flowFstationView),
        ...
        afterRender: function () {
            ...
            if('second-schedule-lt' == this.systemSource){
                if('secondary' == this.dataSource){
                    ...
                    $('#NOTICE_OF_RENT_CONFIRMATION').children('.fish-flowName').text("集客起租");
                }else if('onedry' == this.dataSource){
                    $("#onedryFlow").css('display','block');
                    $('#NOTICE_OF_RENT_CONFIRMATION').children('.fish-flowName').text("一干起租");
                }
            }
            ...
            flowChartVAction.qryParentSubStatusByOrderId(flowStatus,function (data) {
                ...
                if(data != ''
                    ...
                    fish.each(data, function (dataSub) {
                        ...
                        //对本地调度环节进行特殊处理
                        //一干电路如果有完工汇总环节，且环节状态为处理中或者已完成，将本地调度环节点亮成已完成
                        //如果没有完工汇总环节，查询二干本地关联表，有数据将本地调度环节点亮成处理中
                        if('SUMMARY_OF_COMPLETION' == dataSub.TACHE_CODE ){
