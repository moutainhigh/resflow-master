/**
 * 静态流程图展示页面- 二干客户电路新开、变更、拆机流程
 */
define(['module/UnicomLocalNet/resmaster/portal/flowChart/action/flowChartViewAction',
    'text!module/UnicomLocalNet/resmaster/portal/flowChart/templates/secondCustomer.html',
    ...
    return fish.View.extend({
            ...
            flowChartVAction.qryParentSubStatusByOrderId(flowStatus,function (data) {
                ...
                if(data != ''
                    ...
                    fish.each(data, function (dataSub) {
                        ...
                        //本地调度环节特殊处理，判断是否有全程调测环节，且环节状态不是作废
                        //如果有，则点亮本地调度环节为已完成。如果没有查询二干本地关联表是否有数据，有数据
                        //则点亮本地调度环节为处理中
        ...
        initTaskInfoGrid: function($this) {
            ...
            $("#customerView").grid({
                colModel: [
                    //默认展示字段
                    {name: 'TACHE_NAME', label: '工单环节', width: 100, sortable: false},
