    ...
    return fish.View.extend({
        ...
        initContactsGrid: function() {
            ...
            $conGrid =$("#freContactGrid").grid({
                colModel: [
                    //默认展示字段
                    {name: 'ID', label: '联系人Id', width: 180, hidden: true, key:true},
                ...
                onCellSelect:function(e, rowid, iCol, cellcontent){
                    if(iCol != 6){
                        ...
                        meTcon.parentView.freReturnSelect(portalData.ID,portalData.NAME,portalData.OBJTYPE);
                    }
                },
                //双击常用联系人，填充到候选人列表
        ...
        queryContacts: function(page, rowNum, sortname, sortorder) {
            var valueArr = meTcon.options.valueArr;
            ...
            operOrderAction.qrySearchContacts(paramsMap,function (data) {
                if (data.message == "success") {
                    ...
                    debugger
                    if(meTcon.options.multiselect && valueArr != undefined && valueArr.length > 0){
                        $("#freContactGrid").grid("setCheckRows",valueArr,true);
                    }
