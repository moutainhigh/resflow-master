...
define(['module/UnicomLocalNet/resmaster/portal/orderWorkDeal/action/operOrderAction',
    'text!module/UnicomLocalNet/resmaster/portal/orderWorkDeal/templates/assignPersonView.html',
    'i18n!module/UnicomLocalNet/resmaster/portal/i18n/portalView.i18n'
], function(operOrderAction,assignPersonView,i18n) {
    var meT;
    ...
    return fish.View.extend({
        template: fish.compile(assignPersonView),
        ...
        afterRender: function() {
            ...
            multiselect = (this.options.multiselect == undefined)? false:true;
        ...
        //     var nodeValues = meT.options.nodeValues;
        //     var check={};
        //     if(multiselect){
        //         check =  {
        //             enable: true,
        ...
        //                                 }
        //                                 for(var i=0; i<data.length; i++){
        //                                     if(multiselect && data[i].isParent){
        //                                         data[i].nocheck = true;
        //                                     }
        //                                     debugger
        //                                     if(multiselect && nodeValues.indexOf(data[i].id) != -1){
        //                                         data[i].checked = true;
        //                                     }
        //                                 }
        ...
        // initorgPerDeType: function(){ //初始化搜索下的派发类型、值以及改变事件
            // var dataSource;
            // if(this.options.type == "schedule" && currentType=="addAdmin"){
            //     dataSource = [{name: '人员', value: '260000003'}];
            // }else{
            //     dataSource = [
        ...
        initsearchOrgPerGrid: function() { //初始化人员
            ...
            $("#searchOrgPerGrid").grid({
                ...
                pageData: function (start, rowNum, colName, orderType) {
                    me.querysearchOrgPerGrid();
                }.bind(this),
        ...
        querysearchOrgPerGrid: function(page, rowNum, sortname, sortorder) { //岗位、人员、部门查询
            var valueArr = meT.options.valueArr;
            rowNum = 500;
            page = 1;
            //登陆人信息
            var paramsMap = new Object();
            var searchOrgPerName = $("#searchOrgPer").val();//人员、岗位、部门搜索内容
            // var searchOrgPerName = '';//搜索人员
            var orgPerDeTypeVal = '260000003';
            // if(searchOrgPerName == ""
            ...
            operOrderAction.qrySearchOrgPerDepPullDown(paramsMap,function (data) {
                if (data.message == "success") {
                    if("260000001" == orgPerDeTypeVal) { //部门
                        ...
                        if(multiselect && valueArr != undefined && valueArr.length > 0){
                            $("#searchOrgPerGrid").grid("setCheckRows",valueArr,true);
                        }
        ...
        submit:function () {
            if("1" == isOrgSear){
                ...
                if("260000001" == orgPerDeTypeVal){ //部门
                    if(multiselect){
                        selrowPerDepPost = $("#searchOrgPerDepartGrid").grid("getCheckRows");
                    }else {
                        selrowPerDepPost = $("#searchOrgPerDepartGrid").grid("getSelection"); //获取选中的行数据
                    }
                }else if("260000003" == orgPerDeTypeVal){ //人员
                    if(multiselect){
                        selrowPerDepPost = $("#searchOrgPerGrid").grid("getCheckRows");
                    }else {
                        selrowPerDepPost = $("#searchOrgPerGrid").grid("getSelection"); //获取选中的行数据
                    }
                }else if("260000002" == orgPerDeTypeVal) { //岗位
                    if(multiselect){
                        selrowPerDepPost = $("#searchOrgPerPostGrid").grid("getCheckRows");
                    }else {
                ...
                if(multiselect){
                    for(var i = 0; i < selrowPerDepPost.length; i++){
                        var temp = {};
                        temp.id = selrowPerDepPost[i].ID;
                        temp.name = selrowPerDepPost[i].NAME;
                        temp.objType = selrowPerDepPost[i].OBJTYPE;
                        searchArr.push(temp);
                    }
                }else {
                    var searchObj = new Object();
                ...
                var nodes;
                if(multiselect){
                    nodes = $("#transferObjTree").tree("getCheckedNodes");
                    if (nodes.length == 0) {
                        fish.info("请至少选择一个人员！");
                        return;
                    }
                }else {
                    nodes = $("#transferObjTree").tree("getSelectedNodes");
                ...
                var searchArr = new Array();
                if(multiselect){
                    var selrow = $("#freContactGrid").grid("getCheckRows");
                    ...
                    for(var i = 0; i < selrow.length; i++){
                        var temp = {};
                        temp.id = selrow[i].ID;
                        temp.name = selrow[i].NAME;
                        temp.objType = selrow[i].OBJTYPE;
                        searchArr.push(temp);
                    }
                }else {
